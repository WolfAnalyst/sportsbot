...
TIPBOT_VERSION = "v5.52 (2026-06-12: FIVE FIXES from the 06-11 day review + racing reshuffle (Wilson). 1) MLB Alex-single FAST price-change (535) retry: _place_mlb_alex_single retries ONCE per account on the same rung with target_odds dropped instead of abandoning to manual (06-11 Xavier Edwards $34 leftover); ambiguous/slow branch keeps precedence, second 535 stops the account. 2) AusBets/Kev no-unit RAW belt (_raw_has_unit_token; corpus-validated 222 msgs) + ausbets no-bet framing guard (_is_no_bet_framing) -- Groq invented a unit on the 06-11 nothin-today message and Spurs ML placed $400. 3) Image-channel announcement-guard: going-to-send/coming-soon posts are chatter unless urgent (scratch/cancel) or concrete (R4/$50/2u/odds); Eddie 17:39 ping class; actionable log now carries text[:120]. 4) Inert-session crash alerts CRITICAL->INFO: watchdog confirmed-crashed + session-health DOWN/RECOVERED partition by live priority-list membership (session_priority.is_placeable_session, fail-open); placeable stays critical, mixed batches footnote inert. 5) MBL VIOLATION (sports) relabelled BOOKIE STAKE CAP (sports) + Critical->Maintenance (Vassell in-play 538 under-fill was the bookie cap, not ours); label+channel only, gate untouched. PLUS racing priority reshuffle (89c23bc): Sophie tab/lads/betr live at tail, tabtouch dead-last, pointsbet 71269/76341 removed, bet365 53523/73357 pulled pending dev fixes.)"
