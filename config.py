...
TIPBOT_VERSION = "v5.57 (2026-06-13: SOUNDNESS-CHECK FIXES (5-agent verification of v5.54-56, Wilson). 1) reconcile selection gate now TOKEN-BOUNDARY matching (v5.56 substring version false-matched prefixes: ROCK NIEN token rock inside rocknrolldagamawin -- the REAL concurrent pair 62161/62166 from 06-12); both sides tokenise, exact shared token required; shared-whole-word residual documented (ALL-tokens would flip to false-rejects -> racing spill -> double-bet). 2) notifier render guard: reconcile-confirmed placements with no odds/bet_id render @ ? [reconcile-confirmed] not @ None [None]. 3) NOTIFY LOST (final) needle now log-capture-pinned in the suite + plain-fallback-POST-raises path tested. Verifier verdicts: dedup-claim sound, racing/scripts/pacing sound, notifier + confirmed-placed sound-with-nits, reconcile-gate fixed.)"
