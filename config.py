...
TIPBOT_VERSION = "v5.53 (2026-06-12: FAN-OUT AMBIGUOUS TIER-1 RECONCILE + HRR LINE SNAP (Wilson, same-night wire from the 06-12 day review). 1) _reconcile_fanout_ambiguous: BOTH fan-out workers (AFL singles + SGM) now confirm/deny an AMBIGUOUS rung via /api/pending_bets (RECONCILE_AMBIGUOUS gate, spill always OFF) -- placed -> committed at the ACTUAL stake (CONFIRMED wording); not_placed -> clean failure (no debit, no ambiguous CRITICAL, stake surfaces UNFILLED; _reconcile_confirmed_not_placed override in _is_ambiguous_result); conservative/error -> unchanged debit-as-placed. Ladder never continues. Evidence: 18:31 Eddie Geelong o178.5 $250 on 65465 debited maybe-placed + CRITICAL, Wilson manually confirmed never landed. 2) _mlb_hrrbi_to_sgm snaps an impossible INTEGER HRR line {5,15,25,35} -> /10 with a loud warning (Shook 'M 15 HRR' typo = 1.5; Corbin Carroll $400 went manual). 15->1.5 transforms to the 2-leg SGM; 25->2.5/5->0.5 stay singles->manual; nothing else snapped.)"
