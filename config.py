...
# below). SHIPS MANUAL-ONLY -- Stage 1 (main._route_image_nfl_tips) never
# places and does NOT read this value at all yet; it exists only so
# TIPSTER_CHANNELS has a unit_size to carry, ready for Stage 2
# auto-placement. $1 placeholder default until Wilson has seen enough real
# 4th&EV images and sets a real number in .env; deliberately NOT copying
# Eddie/Zak's production defaults so a future auto-place wire-up can't
# silently inherit a real stake before Wilson has chosen one.
FOURTHANDEV_UNIT_SIZE = _env_float("FOURTHANDEV_UNIT_SIZE", "1")

# A1 Fantasy Sports NFL (2026-09-10, Wilson): a SEPARATE tipster from
# 4th&EV above, TEXT-only, group -4890384626 / bot 8024788433. SHIPS
# MANUAL-ONLY (main._route_a1_nfl_text never places) for the same reason as
# 4th&EV: never automated before (checked -- zero prior code/git history),
# and A1's real format is genuinely nuanced (alt-book line variants,
# conditional fallback markets in prose) so it needs validating against
# more real samples before any auto-place wiring.
...
TIPSTER_CHANNELS = {
    ...
    # image-tips note above). SHIPS MANUAL-ONLY: main._route_image_nfl_tips
    # sends every extracted tip to a manual alert and never calls place_tip.
    # IMAGE_PROMPT_NFL (groq_parser.py) and the market mapping are validated
    # against ONE real 4th&EV card so far (a branded BET/UNITS/BOOKIE-labelled
    # graphic) -- the same bootstrap Eddie/Zak/Trial went through before their
    # auto-place paths were built and turned on.
    -1004359137317: {
        "name": "4th and +EV NFL",
    ...
    # image channels above), TEXT tips, never automated before (checked: zero
    # prior code/git history for "a1_fantasy"/-4890384626/8024788433). SHIPS
    # MANUAL-ONLY: main._route_a1_nfl_text sends every extracted tip to a
    # manual alert (full raw text + a best-effort headline) and never calls
    # place_tip. Routed to its OWN pipeline (main.py's dispatch checks
