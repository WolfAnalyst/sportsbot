...
TIPBOT_VERSION = "v5.3 (2026-06-03: IMAGE-TIP FIXES on top of v5.2. (a) Ante-post racing date carry: _img_parse_racing_date converts the vision date (weekday 'Saturday'->next Sat / ISO / D-M / 'June 6' / today-tomorrow) to ISO anchored on the post; _build_racing_tip_dict threads it to price_check/place so Zak's SATURDAY card isn't price-checked against today (was -> 'race not in catalog' -> manual + wrong-day match risk). Cache logs proved meetings ARE cached from ~midnight, so it was a date bug not market timing. (b) Eddie AFL totals+team-lines now auto-place (were always manual): _build_afl_tip_from_image emits total_points (over/under, catalog +-1.0) + line (signed handicap, catalog +-0.5 via _match_handicap_in_catalog, never wrong side); margin/other still manual; all $1-gated. total_points AFL catalog key wants live confirmation next slate. (c) Vision over-parse guard: eddie_afl prompt now extracts only the highlighted bet (not the background odds grid) + _image_afl_conflicting_indices routes over+under-of-same-line pairs to manual so both sides can't be staked. Builds on v5.2.)"
