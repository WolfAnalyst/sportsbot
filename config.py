...
# A1 NFL (2026-09-10, Wilson): onboarding a new image-tip tipster. SHIPS
# MANUAL-ONLY -- Stage 1 (main._route_image_nfl_tips) never places and does
# NOT read this value at all yet; it exists only so TIPSTER_CHANNELS has a
# unit_size to carry, ready for Stage 2 auto-placement. $1 placeholder
# default until Wilson has seen real A1 images and sets a real number in
# .env; deliberately NOT copying Eddie/Zak's production defaults so a future
# auto-place wire-up can't silently inherit a real stake before Wilson has
# chosen one.
A1_NFL_UNIT_SIZE = _env_float("A1_NFL_UNIT_SIZE", "1")
...
TIPSTER_CHANNELS = {
    ...
    # A1 NFL (2026-09-10, Wilson): "NFL Tips from 4thandEV", set up the same way
    # as Eddie AFL -- image-tip channel, no bot_id (broadcast channel, sender
    # filter intentionally skipped, see the image-tips note above). SHIPS
    # MANUAL-ONLY: main._route_image_nfl_tips sends every extracted tip to a
    # manual alert and never calls place_tip. We have not yet seen a real A1
    # tip image, so IMAGE_PROMPT_NFL (groq_parser.py) and the market mapping
    # are unvalidated -- exactly the same bootstrap Eddie/Zak/Trial went
    # through before their auto-place paths were built and turned on.
    -1004359137317: {
        "name": "A1 NFL",
        "parser": "a1_nfl",
        "default_units": 1.0,
        "unit_size": A1_NFL_UNIT_SIZE,
        "sport": "nfl",
        "image_tips": True,
    },
}
