...
TIPBOT_VERSION = "v5.54 (2026-06-12: CONCURRENT TIP TITANS BATCH + CROSS-TIP SESSION PACING (Wilson). 1) tiptitans_processor._process_batch: a poll batch now places CONCURRENTLY (asyncio.gather bounded by TT_CONCURRENT_TIPS=4; 1 = old sequential) -- TT releases tips simultaneously and the sequential loop bet later tips into stale prices (06-12 Redcliffe ~6min lag, odds-too-low on the later tips). M23 evolved: ids marked seen + persisted BEFORE tasks start (mid-batch crash/restart must never double-place an in-flight tip; crashed tip = H27 alert, not retried). 2) racing_placer cross-tip same-session pacing: consecutive placements on ONE session_id are spaced by a RANDOM TT_SAME_SESSION_GAP_MIN..MAX (5-10s) gap ACROSS tips (reservation pattern, no lock held during placement; PLACED re-books from landing time; ladder rungs immediate; balance rejects spill as usual; GAP_MAX=0 kill-switch; covers TT + image racing). WATCH first live slate: TAB sidecar/bet365 load vs the 1-failure circuit breaker.)"
