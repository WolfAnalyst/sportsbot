...
TIPBOT_VERSION = "v5.94 (2026-07-05: AFL REDISTRIBUTE MULTI-PASS (Wilson, from the 07-04 failure review). The AFL fan-out unfilled-remainder redistribute (v5.78/79) was a SINGLE even-split pass over the base-pass successes, disposals-OVERS only -> two money leaks on 07-04: (A) a top-up leg that itself re-403'd on a dead-proxy session (102506/65463) had its crumb ABANDONED to manual (~$143 over Connor Macdonald/Shai Bolton/Cripps); (B) it NEVER fired for under/total/goalscorer markets, so a single-account 403/508 dropped the whole failed share (~$1,267 over Jye Amiss goals / Hawthorn total / Jack Crisp under). NOW _afl_redistribute_topup is a BOUNDED MULTI-PASS (AFL_REDISTRIBUTE_MAX_PASSES=3): pass 0 splits the remainder across the base-pass successes; each later pass re-splits whatever is STILL unfilled across ONLY the accounts whose top-up JUST SUCCEEDED (proven-healthy), so a re-403'd crumb re-shops onto a working sibling instead of dropping to manual. A session that fails pre-placement (403/auth) is EXCLUDED from later passes (a dead proxy won't recover mid-tip); if the even share falls below the bookie $min it CONCENTRATES onto the fewest accounts that each clear the min (no sub-min fragmentation); it stops cleanly when remainder < min / no healthy target remains / a pass places nothing / max_passes. The call-site GATE is WIDENED: redistribute now also fires for any single-account NON-CAP failure (403/auth/508) on ANY market (not just disposals-overs) -- a proxy/funds failure is not a moved-line risk so the v5.79 overs-only guard doesn't apply; a pure stake-cap (538) miss on a non-overs market still does NOT redistribute (whole book capped, no headroom). MONEY-SAFE: remainder decreases each pass by the COMMITTED stake -- clean placed PLUS any maybe-landed/ambiguous top-up (treated as committed so a landed-but-uncertain leg is NEVER re-shopped onto a sibling) -- so base + top-ups can never exceed the unit (no over-stake, no ambiguous double-stake); bounded passes. 10-OPUS REVIEWED: the review caught a real over-stake BLOCKER (a maybe-landed top-up's slice was left in remaining and re-shopped the next pass -> tip could exceed its unit if the ambiguous bet landed) -- FIXED by committing the ambiguous at-risk stake into remaining (mirrors the call-site ambiguous_total) + added the test_v594 scenario-B regression. NOT touched: the SB player-disposals liability caps (Wilson is scoping the ~halved SB limits himself; the 07-04 by-design ~$5.4k cap unfill is unchanged). ALSO: bet365 Daniel 73359 REMOVED from RACING_SESSION_PRIORITY (.env) -- it prices fine but every PLACE returns 'Bet placement failed' (bet365 cp=False / sr=0 mi=none re-confirm reject + websocket/uqid-cookie errors on that session); Alex 73361 places fine (dev bundle in BET337 for HB). +test_v594_redistribute_multipass; suite green. main.py/config.py/.env; requires main.py restart. --- v5.93 (2026-07-03: NEW TIPSTER 'Late Mail Leroy' -> BETFAIR BSP (Wilson, 10-opus reviewed). A new Telegram CHANNEL (id -1003535060447), Thoroughbreds across Australia (no set state), where EVERY post is treated as a tip and noise is filtered by the parser (parsers/leroy.py returns None on chatter/no-runner). He tips a saddle NUMBER (no horse name) + units, e.g. 'Rosehill R5 / #1- 2 units / #2. 0.5 unit W. 1.5 unit Place'; a 'W / Place' line stakes a WIN portion + a PLACE portion, a plain 'N units' is all WIN. These place via BETFAIR EXCHANGE BSP (back at the Starting Price) on the dedicated Betfair session 99997 -- a BRAND-NEW placement path, NOT the fixed-odds racing price-shop. hyperbot_client.place_betfair_bsp posts /v3/place_bet with category=betfair, bet_direction=back, bsp=true, market=win|place, track+race_num+runner NAME (HB resolves the Betfair market -- no IDs), size=stake (Erasmus-safe max_attempts=1, a BSP order can't be double-submitted). SADDLE#->NAME is resolved off a price_check_racing card (saddle numbers are identical across bookies): the Betfair session first, then a bounded fall-back through the first 3 racing-priority sessions if Betfair serves no numbered card. STAKE: $LEROY_UNIT_SIZE=400/unit, FULL stake NO splitting across accounts, capped at LEROY_MAX_UNITS=4 PER BET ($1,600 max; win + place count separately). A resolved leg is placed + ledgered (bet_ledger.log_racing_bet, bookie=betfair, odds blank -- BSP unknown until the jump) + surfaced in a new notifier.notify_betfair_bsp_placed 'BET PLACED (Betfair BSP)' Bet-Log msg; an UNRESOLVABLE saddle or a failed place routes THAT leg to a manual alert (never a wrong/double bet). Gated LEROY_ENABLED (kill-switch, default true). New files: parsers/leroy.py; new config LEROY_UNIT_SIZE/LEROY_MAX_UNITS/LEROY_BETFAIR_SESSION/LEROY_ENABLED + a TIPSTER_CHANNELS entry (betfair_bsp:True, no bot_id = a channel); main._process_leroy_betfair_tip + the betfair_bsp handler branch; sessions.yaml 99997 (owned/monitored, NOT in any *_SESSION_PRIORITY). +test_v593_leroy_parse_and_bsp (parse plain/split/noise + 4u cap + both markets + manual fallback). WATCH the FIRST live Leroy placement -- Betfair BSP is untested in production; OPEN UNKNOWN = whether HB serves a separate 'place' BSP market (if not, the Place portion just routes to manual, money-safe). 10-OPUS REVIEWED (initial verdict BLOCK): the review found + I FIXED 4 VERIFIED money-safety BLOCKERS before ship -- (1) NO DEDUP: the create_task'd Leroy path registered no fingerprint, so a telethon reconnect catch-up re-delivery / a re-post would double-back (up to $3,200 on a 4u tip); added _leroy_recent_fps keyed (LEROY,track,race,date,saddle,market), registered BEFORE the place, day-long TTL (a BSP race resolves hours later, so the 10-min racing window was too short). (2) AMBIGUOUS-> 'place manually': a POST/cid-timeout returns {'success':False,'ambiguous':True} (the back MAY have matched at SP) but the handler treated it as a plain fail and told Wilson to place it by hand = double-stake; now an ambiguous result fires a verify-the-account CRITICAL and NEVER a 'place manually' (mirrors racing_placer). (3) RAW UTC DATE: date_str used event.date (UTC) not AEST, so a morning late-mail tip fetched the PREVIOUS day's card and placed a wrong-numbered runner; now anchors on the post's AEST day (+10h, matches _img_parse_racing_date). Leroy is DAY-OF ONLY -> always the same-day card, NEVER roll a date forward. (4) DUPLICATE SADDLE LINE: the parser appended per-line + the 4u cap was per-line, so a repeated '#1' double-backed; parser now COLLAPSES duplicate saddles per race (MAX units) and the per-(saddle,market) dedup fp closes it too. Plus review nits: multi-race grouping (a 2nd header no longer folds runners into race 1), track alias-normalised, top-level crash-guard (a create_task exception pings manual not a silent drop), 'u' shorthand + header-but-0-legs -> manual (never lose a real tip), empty-bet_id ledger synth key, units :g format, empty-text else. Parser return changed to {'races':[...]}. +test_v593 expanded (parse/dedup/ambiguous/AEST/synth-id). Requires main.py restart + sync to sportsbot (shared code; the fork ignores the Leroy channel/session/.env). RE-VERIFIED by a 2nd 10-opus pass -> 2 MORE blockers found+fixed: (5) my CHANGELOG edit had put a literal races-dict with double-quotes INSIDE the double-quoted TIPBOT_VERSION -> SyntaxError -> the WHOLE bot couldn't start (first py_compile passed only because that edit came after it); fixed (single-quoted) + added a py_compile smoke-gate to deploy.ps1. (6) a MAYBE-LANDED BSP (POST got a correlation_id but every bet_status poll then failed) returned success=False with NO ambiguous flag -> fell to 'place manually' = double-back; place_betfair_bsp now tags that cid-carrying envelope ambiguous, AND _leroy_place_race adds the standard slow-rejection guard (elapsed>=STAKE_REJECT_LATENCY_THRESHOLD_SEC and not _is_definitely_pre_placement -> maybe-landed -> CRITICAL never manual). Plus leading-dot decimals (.5u->0.5 not 5.0; the _SADDLE_RE separator was eating the decimal point, the saddle-separator regex no longer eats it), per-race crash isolation, and chatter with a bare R<n> but no #-line now drops as noise. +5 tests. A 3rd focused 10-opus pass verified all round-1+round-2 fixes closed with runtime checks -> SHIP_WITH_NITS (no blockers); fixed NIT-1 (a malformed race sharing a post with a well-formed race was dropped silently -> parser now tracks the #-line flag PER race + emits a legs=[] entry, dispatcher fires a per-race manual alert; the good race still places). Parser returns None (noise) or {'races':[...]} where legs=[] = surface-as-manual. 2 residual NITs tracked (pre-existing, non-money-safety): glued-dot '#1.5u' no-space -> 5.0u (cap-bounded, out-of-format) + a defensive note on the currently-unreachable per-race crash handler. --- v5.92 (2026-07-03: 06-28..07-02 review batch (Wilson) + Aditya account. (1) ZAK UNDATED auto-recovery ATTEMPTED then REVERTED (10-opus review BLOCKER): running the undated date-resolver for a next-day tipster even with today-prices would, for a GENUINE same-day Zak card (Zak DOES post same-day, e.g. Bordertown 06-28 ~$2k), roll forward to a WRONG date + via the saddle-fallback a WRONG horse -- strictly WORSE than the original, which is MONEY-SAFE (the 06-30 phantom -> HB 'Could not determine event ID' -> manual, no bet). REVERTED (racing_placer net-unchanged); the proper fix is PLACE-TIME (detect the event-not-found -> roll the date -> re-place) as a follow-up. (2) EDDIE WEEKEND-RECAP guard (_IMAGE_RECAP_RE): the 06-28 'Meh weekend ... (-5.8u by my calculations)' + 07-02 '2-2 night ... cooking us' P&L wrap-ups fell through to the units pattern (the -5.8u P&L read as a stake) -> false manual ping; the recap regex now short-circuits them (real forward tips still ping). (3) 403 RE-BET NOTES (Wilson): a fan-out account that hits a proxy-403 (or other transient pre-placement reject) on the first attempt but RECOVERS on the same-stake re-bet now tags the bets_placed.csv `note` column + the BET PLACED summary ('sportsbet:X 403 proxy error -- fixed on re-bet'); an account that FAILS TWICE (403 both attempts) is flagged in the manual/UNFILLED alert ('X -- FAILED TWICE on this message'). The retry itself is the existing v5.68 single same-rung retry (debit-safe -- 403 = clean no-debit pre-placement reject). (4) ACCOUNT: added Aditya sportsbet 111463 -- SPORTS / AFL-SINGLES ONLY, caps mirror Wilson 53522, SGM + boosts DISABLED, NOT racing; wired into AFL_SESSION_PRIORITY only, so it adds concurrent Eddie/Saiyan AFL-singles capacity (raises total placeable stake per tip). +tests (test_v592_*; suite green). 10-OPUS REVIEWED: item (1) was a wrong-date/wrong-horse BLOCKER on a genuine same-day Zak card -> REVERTED (racing_placer net-unchanged); (2)(3)(4) SHIP + the review nits fixed (recap 'N-N night' now dash-gated + 'cooking us' dropped; _prob 403-label tightened to require a proxy/forbidden/client-error marker). main.py/bet_ledger.py/notifier.py/config.py/sessions.yaml/.env; requires main.py restart + sync to sportsbot (shared code; fork ignores 111463/.env). --- v5.91 (2026-06-28: 06-27 review batch (Wilson), 10-opus reviewed. (A) CONCURRENT IMAGE TIPS: the N tips in a SINGLE Saiyan/Eddie image now place CONCURRENTLY -- _route_image_afl_tips rebuilt into 3 passes: PASS 1 build+dedupe+audit SEQUENTIAL on the main thread, PASS 2 place_tip in a bounded ThreadPoolExecutor, PASS 3 register-fingerprint+log SEQUENTIAL. 06-27 Crisp/De Goey/Trezise = 3 tips processed sequentially -> 76s for the 3rd; now ~max-single-tip. Thread-safe: bet_ledger already locks its CSV; _log_jsonl now wrapped in _AUDIT_LOCK (concurrent tip_outcome appends); _recent_tips only touched in PASS 1/3 on the main thread; _line_move_retried_sessions is per-tip. Batch-dedupe (_batch_fps) preserves the check-then-register-sequentially semantics; one tip's place_tip crash is ISOLATED (siblings still place + the crashed one pings manual). Gated IMAGE_TIP_CONCURRENT (default true) + IMAGE_TIP_MAX_CONCURRENCY=2 (SOFT LAUNCH, raise once verified on a live multi-tip slate + the proxy is fixed). (B) the AFL fan-out 403s on the two PROXIED Sportsbet/bet365 accounts (Ryan .80 / Alex .228) root-caused to the shared Vocus proxy provider intermittently FORBIDDING the bet POST per-IP under the concurrent burst (POST-only; GETs clean; money-safe clean no-debit rejects) -- PRIMARY fix is OPS (dedicated IPs / de-proxy the AU Sportsbet accts), Wilson's call; NO tipbot code change this batch. (C) the 0-tip image manual ping no longer falsely says 'Image bet detected' -- new notifier.notify_image_no_tip ('image received but no bettable tip found') for the parse-returned-0 path (the 06-27 23:33 Eddie recap image); still PINGS (a fumbled real tip must not be silently dropped under Claude-primary). (D) a trackless ZAK (SA-only) tip now resolves reliably: the SA-track web-search resolver gets the SA candidate-track list (fast CONSTRAINED lookup) + ONE retry on empty, and claude_parser._websearch_complete caps SDK max_retries=1 + timeout=20s (was 40s x default-2-retries = ~122s that froze the event loop on 06-27 'Pure Bliss' -> track=None -> manual). A wrong resolved track still fails the NAME match + odds floor downstream -> manual. (E) an Eddie MONEYLINE/MULTI/PARLAY image (no market_type slot in IMAGE_PROMPT_AFL) was returning tips=[] and the manual ping lost the bet detail; the prompt now emits a market_type='other' tip carrying a plain-English `description`, and _route_image_afl_tips routes a description/'other' tip to a MANUAL alert WITH the bet text (NEVER auto-placed). +3 tests (test_v591_*). 10-OPUS REVIEWED (SHIP-WITH-NITS, no money-safety blocker -- the one BLOCKER claim, concurrent reconcile cross-confirm, does NOT apply to AFL: reconcile matches player-name tokens so two disposal props can't cross-confirm, + a same-player twin is collapsed by _batch_fps): narrowed the parlay guard to market_type other/empty only (#3, was over-firing on a totals tip with a stray description), wrapped PASS-3 per-tip so a post-place crash can't drop a sibling's fingerprint (#5, the only residual double-bet vector), added a web-search wall-clock cap _WEBSEARCH_BUDGET_SEC (#4, the loop could run turns x 40s not 40s). main.py/config.py/notifier.py/claude_parser.py/groq_parser.py; requires main.py restart + sync to sportsbot (shared sports code + the IMAGE_PROMPT_AFL prompt). --- v5.90 (2026-06-27: two SAFE fixes from the 06-27 Saiyan-tips review (Wilson). (1) LATENCY -- the BUG-A Claude web-search backstop (~14s/leg) was MIS-FIRING on EVERY already-rostered AFL player: resolve_player_name returns the name UNCHANGED both on a true roster miss AND when it was already exactly correct (Saiyan/Eddie send full names like 'Jack Crisp'/'Elliot Yeo'), so the old player==_before trigger web-searched all of them. 06-27 Crisp/De Goey/Trezise (3 tips in one image, processed SEQUENTIALLY) each paid ~14s x N legs -> 76s for the 3rd tip. Now: skip the web-search when the player's roster-inferred team (leg.team_full) already matches one of the fixture's teams (player is rostered AND in this game -> backstop redundant). A genuinely stale-roster player (team_full empty or off-fixture) still web-searches, so the BUG-A wrong-game backstop is intact. Cuts the dominant per-tip latency without changing which bet places. (2) FALSE 'BET UNFILLED' MANUAL ALERT -- the AFL + ETR fan-out fired the manual/unfilled alert on (failed_results OR unfilled>1.0), so an account failure that the overs-redistribute (or ladder) FULLY covered still pinged 'manual' even though the unit placed in full. 06-27 Jack Crisp: Ryan 102506 proxy-403'd, redistribute topped the $120 onto the 3 working accounts (unfilled -> $0.00), bet FULLY placed, but a BET UNFILLED alert fired anyway. Now gates on unfilled>1.0 ONLY (nothing to place by hand when unfilled<=$1; the BET PLACED summary + daily review still surface the failed/recovered account). +2 tests (test_v590_unfilled_alert_gate, test_v590_websearch_skip_when_rostered; full suite green). main.py only; requires main.py restart. NOT changed (Wilson decision pending): the Saiyan Baker+Duggan SGM rejected 'combined 1.44 < target 1.62' -- a same-team CORRELATED SGM Sportsbet priced 1.44 vs the tipster-quoted ~1.80 (90% floor=1.62); the SGM odds-floor for correlated SGMs is a value call. Ryan 102506 + Alex 65463 + bet365 LLPunts still on the dead 212.69.0.x proxy (403) -- ops, not tipbot. --- v5.89 (2026-06-26: SPORTSBOT FORK SGM-UNIT SAFETY (Wilson). SAIYAN_SGM_UNIT_SIZE now DEFAULTS to SAIYAN_UNIT_SIZE (the single unit) instead of a hardcoded 750. The sportsbot fork sets NO unit overrides in its .env, so it was inheriting Wilson's full $750 Saiyan-SGM unit while its singles defaulted to $150 -> the friend's bot placed $750 on a Saiyan SGM by default. Now a fork with no override stakes its SGM at its single unit ($150). TIPBOT IS UNCHANGED -- its own .env sets SAIYAN_SGM_UNIT_SIZE=750 explicitly, which still wins; only an unset default changed. The fork picks it up on its next pull + restart. config.py only. --- v5.88 (2026-06-26: 3-day BOOKIE-ANALYSIS ops fixes (Wilson, 'fix it all and restart') + lands the uncommitted 06-25 WIP that was on disk. BOOKIE-ANALYSIS (logs/BOOKIE_ANALYSIS_3DAY.md, 7-agent): ledger money-safe / 0 double-bets / reconciles to the cent; actionable tipbot fixes: (1) PULLED 3 dead/off-ledger-risk sessions from RACING_SESSION_PRIORITY -- tab:78280 (Daniel TAB, 'Intercept handling failed' x4 -> off-ledger trader-review, out until HB BUG-001352 ships), tab:100003 (Sophie TAB, 0 placed/dead), bet365:73359 (Daniel, 0 fills / 35 price-fails over 3 days, trips the CB = dead weight; bet365 confirm path broken/limited). (2) M1 MONEY-SAFETY (racing_placer): a TAB 'Intercept handling failed' is now AMBIGUOUS (maybe-landed) and NEVER spilled -- the trader-review bet often lands minutes later, outside the ~20s reconcile window, so the old confirmed-absent->spill DOUBLE-STAKED (06-19 Shes Crystal $1,052.50 + Hale Saint Louie $200 off-ledger). Now record ambiguous (verify-account CRITICAL), debit remaining (no spill, no re-place), blocklist, STOP. Always-on guard (independent of RECONCILE_AMBIGUOUS); reverses the 2026-06-18 'leave the spill' decision now the $1.25k double-stake materialised. +test_racing_intercept_no_spill (never spills even when reconcile says 'spill'). Remaining bookie-analysis items are OPS/manual, routed to Wilson (manually reconcile Daniel TAB 78280; top up tab:71265; proxy/508 health-check sportsbet:65463; raise HB INSUFFICIENT_FUNDS/508 mislabels). Actions 4 (EasyMoneyAFL 'Richards' fixture-scope) + 8 ($1 MLB-single floor) already shipped v5.86/87. --- BUNDLED 06-25 WIP (was uncommitted on disk; both test suites green incl. test_2026_06_25_fixes.py): MLB same-full-name COLLISION guard (roster_mlb.json '__collisions__' block + curated map; resolver refuses a blind team-override/inference for a colliding fullname -> manual; fixes Pete Alonso / Max Muncy / Vlad Guerrero wrong-game placements) + PSEUDO-SGM demotion (Groq splitting a same-PLAYER tip into a fake 2-leg SGM -> demoted; Saiyan McCartin) + MLB_HRRBI_LADDER_PCT extended ...0.8 -> 0.8,0.7,0.6 (06-23 Olson $105.40 bookie per-line cap was under the old 0.8 bottom rung -> whole SGM $0->manual; lower rungs let an account place below a tight cap; purely additive) + GWS GIANTS / GREATER WESTERN SYDNEY GIANTS compound AFL_TEAMS mappings (vision logo tips) + roster/parser/notifier edits per the 06-25 diff. (stale test_v574 updated to assert the 6-rung ladder.) Requires main.py restart. --- v5.87 (2026-06-22: 10-opus-review follow-up -- the 2 deferred MAJOR items, fixed (Wilson). (1) BUG A extended from AFL SINGLES to AFL SGM LEGS: the SGM leg resolver (_enrich_sgm_legs_with_prop_ids + the _place_sgm/_place_sgm_fanout leg loops) now passes event_teams=both fixture teams into _resolve_leg_for_hyperbot, so a bare surname on an SGM leg scopes own-team-FIRST then to the two event teams and NEVER uses the cross-game global roster fallback -- closes the same wrong-GAME-player hole on the SGM path that v5.86 closed for singles (AFL SGMs are same-game, so scoping to both teams is always correct). Non-AFL SGM (MLB/NBA) unchanged (event_teams=None). (2) VISION hard-fail silent-drop CLOSED: parse_tip_image_claude now RAISES on a HARD failure (API/request error, no content, JSON-repair fail, malformed 'tips') instead of swallowing to ([],t) -- mirrors parse_racing_text_claude. Under CLAUDE_PRIMARY the _process_image_tip try/except then routes the unparseable image to MANUAL, instead of the summary/recap 0-tip suppressor silently DROPPING a hard-failed image whose caption looked like a recap (the documented v5.85 known follow-up). A CLEAN valid-but-empty parse still returns [] (real chatter -> correctly suppressed); no-key/no-image still ([],0.0). +2 tests (SGM event-team wiring + image hard-fail raises). DEFERRED by Wilson (kept active, decide later): TAB BUG-001352 HB-deploy verify, CLAUDE_WEBSEARCH_RESOLVE inline-blocking hardening, AFL fan-out no-price-floor residual. Requires main.py restart. --- v5.86 (2026-06-22: BUGS A-E + MINOR multi-fix (Wilson), then a 10-opus MAX-effort review. (A) AFL surname WRONG-GAME fix: an AFL player surname is now scoped to BOTH teams of the resolved fixture (fuzzy_match_player(teams=[...])) and the cross-game GLOBAL roster fallback is FORBIDDEN when so scoped -- 06-21 'afl Richards over 25.5 disposals, St Kilda v Western Bulldogs' had scoped 'Richards' to STK only, missed, then the global fallback grabbed Joe Richards (Port Adelaide, NOT in this game) instead of Ed Richards (Western Bulldogs); only the catalog miss saved a wrong bet. Now resolves Ed Richards uniquely; if neither event team has the surname it stays the bare surname for the event-scoped catalog matcher (or, for a FULL name absent from the stale roster, a Claude web-search backstop confirms the club IS one of the two event teams before proceeding) -- NEVER a wrong-game player. _resolve_single_for_placement + _place_afl_fanout derive event_teams from tip.event. (B) NON-BET messages no longer hit MANUAL: (1) an Eddie image-channel RECAP/commentary text ('Not the best weekend... copped some tough beats', 'Have taken these combos as SGMs, could be better odds elsewhere', 'Results:') now DROPS silently even when it brushes a stray score/odds -- _image_text_is_actionable gained a recap guard (summary/result/commentary signatures) that suppresses UNLESS an urgent scratch/cancel instruction is present; (2) an Eddie AFL image whose tip has NO odds (the MULTI/SGM-leg signature -- 12:52 Sheezel/LDU/Short @?, 12:56 Wardlaw @?) DROPS quietly with no 'N tips in one image' MANUAL BET ALERT (the user does not take Eddie multis); a single WITH odds still places (incl. the lone no-units 2.5u fallback); (3) the Shook 0-tips parse-error gate now requires a CONCRETE bet signal (unit token / @odds / HRR-HRRBI line shape) AND not no-bet framing instead of '@everyone + any digit' -- the 06-21 20:05 'No alt for Baldwin, I also sprinkled 2+ Hits... if u wanted' chatter (whose @everyone came from a prepended CONTEXT msg) no longer false-fires PARSE ERROR. (C) AFL auto-place price tolerance WIDENED to 15% when the tipped price is > $2.00 (else 10%); racing's ODDS_DRIFT floor untouched. New _afl_target_odds(sport, basis) is the single floor source across every AFL target_odds site (SGM/single/fan-out) + the apply_floor breach gate: 2.00->1.80, 2.01->1.71, 2.16->1.84 (Ryley Sanders u23.5 @2.16 now ACCEPTS the 1.87 market that the old 1.94 floor rejected on all 5 accounts), 3.00->2.55; non-AFL (MLB/NBA) unchanged at 10%. Only LOWERS the floor (accepts shorter odds = lower liability) -- never raises risk. (D) MLB player GUARDED FUZZY resolve: roster.mlb_fuzzy_player corrects a near-identical full-name typo ('Jung Ho Lee' -> roster 'Jung Hoo Lee', SF Giants, 0.957) so the FIXTURE resolves (was 'No fixture found' -> manual). FULL-STRING similarity ONLY (no surname-boosting, which would drift 'Juan Soto'->'Gregory Soto'), threshold >=0.9, a 2+ token query (never a bare surname), and resolves ONLY when EXACTLY ONE roster player matches -> can fix a typo but never pick a wrong-team same-surname player; rewrites the typo to the roster spelling on every leg (the HRRBI SGM has 2 same-player legs). (E) PARSE-ERROR label now reflects the ACTUAL parser ('Claude returned empty tips' under CLAUDE_PRIMARY, not the hardcoded 'Groq...'). MINOR: the MLB HRRBI 3-way SGM fills the full $400 ($133.33x3=$399.99), leaving $0.01; the spillover single now requires MLB_HRRBI_SINGLE_MIN_STAKE (default $1) so a rounding remainder no longer fires a degenerate ~$0.01 bookie-minimum single (06-21 bets 6892885141/6892887454, recurring). SCOPE NOTE: BUG A's event-team scope + no-global-fallback is wired on the SINGLES path (_resolve_single_for_placement / _place_afl_fanout); AFL SGM legs (same-game, per-leg team) keep the prior team-scoped+global-fallback resolve. +5 regression tests (test_v586_*). Requires main.py restart. Also produced a read-only 3-DAY (06-19..21) per-bookie price/place/failure analysis (logs/BOOKIE_ANALYSIS_3DAY.md): money-safe, 0 double-bets, reconciles to the cent; one OPEN real-money item = the 06-19 TAB trader-review intercept ~$1,252.50 off-ledger on Daniel TAB 78280 (HB BUG-001352, needs manual reconciliation). --- v5.85 (2026-06-21: 2nd 5-opus review fixes (Wilson). The v5.84 fixes got their own 5-opus xhigh review (verdict: BLOCKER genuinely CLOSED, no new bug, NOT rollback) -- but it flagged 3 real items now fixed: (1) HIGH: parse_racing_text_claude did NOT strip no-runner rows like groq_parser.parse_racing_text does -- under CLAUDE PRIMARY a model-emitted runner-less-but-SADDLED row ({saddle:7,runner:null}) survived the router guards and would place the WRONG horse via the racing_placer Pass-3 saddle-only fallback; now mirrors Groq's strip + raises on a top-level non-object (was an AttributeError). (2) update_afl_roster.ps1's v5.84 'git checkout -- roster_afl.json' restore was a NO-OP -- the corrupt scrape is git-add'ed before the blocked commit, so checkout restored the CORRUPT staged blob from the INDEX; now 'git checkout HEAD -- roster_afl.json' restores the clean COMMITTED roster. (3) tip_parser fallback-wrapper docstring corrected: parse_racing_text_fallback RAISES by design (must be wrapped) while the text/image wrappers return sentinels. +test (no-runner strip). KNOWN follow-ups (review-confirmed money-safe + backstopped): parse_tip_image_claude still swallows hard failures (image hard-fail with a summary-matching caption could silent-drop -- no-caption images still ping manual); web-search resolvers run inline (40s timeout caps it). Requires main.py restart. --- v5.84 (2026-06-21: 5-opus review fixes for v5.83 (Wilson). BLOCKER FIXED: parse_racing_text_claude now RAISES on a HARD failure (Claude API error / JSON-repair fail) + uses the dedicated TEXT_PROMPT_RACING (was wrongly the image OCR prompt) -- under CLAUDE PRIMARY a transient Claude failure on a real Zak/Trial racing TEXT tip was silently dropped as 'chatter' (no manual alert, no Groq backstop); now it propagates -> the caller routes to MANUAL exactly like Groq-primary did ('never lose a real tip'). Also: (a) the 4 Claude fallback-RETRY branches now skip when CLAUDE PRIMARY already parsed with Claude (was a redundant 2nd Opus call + a 2nd full image upload on every genuine 0-tip miss); (b) update_afl_roster.ps1 RESTORES the clean committed roster (git checkout) when the pre-commit gate blocks a corrupt scrape, instead of leaving the corrupt file on disk to load at the next restart (the 06-21 mojibake incident); (c) web_search resolver calls bounded by a 40s SDK timeout (they run inline on the event loop; a full run_in_executor offload is a tracked follow-up). +test. Requires main.py restart. --- v5.83 (2026-06-21: CLAUDE PRIMARY (Wilson: 'everything through claude, groq off'). The parser now SKIPS Groq entirely and parses EVERY tip (text / vision / racing-text) with Claude (Opus) up front -- gated CLAUDE_PRIMARY=true in .env. Rationale: Groq llama-4-scout is deprecated (shutdown 07-17) + gibberish-prone, and the Groq-fail-THEN-Claude-fallback round-trip added too much latency before a tip could place. PURE PARSER SWAP: every tip still flows through the SAME no-bet/units/roster/floor/dedup/placement gates -- no new bet risk (the no-bet guards still drop chatter; etr blind path unchanged from when Groq parsed it). FORK-SAFE: CLAUDE_PRIMARY defaults FALSE and _claude_primary_enabled() falls through to Groq when Claude is unavailable (no key/SDK), so the sportsbot fork (no key) stays on Groq. Inert under TIPBOT_TESTING (the unit suite keeps using the mocked Groq path). Groq stays wired as the if-Claude-unavailable path + is re-enabled by flipping CLAUDE_PRIMARY=false. Requires main.py restart. --- v5.82 (2026-06-21: Claude fallback EXTENDED to the regex-tipster double-fail path (Wilson). v5.80's text fallback only covered Groq-ONLY tipsters (Shook); a REGEX tipster (Saiyan AFL) whose Groq AND regex parser BOTH fail on a bet-looking message went straight to manual with NO Claude attempt. LIVE MISS 2026-06-21 09:00: a real Saiyan 2-leg SGM (Luke Davies-Uniacke 24+ / Seth Campbell 13+ disposals @1.88) was lost this way to a Groq invalid-JSON GIBBERISH failure (same llama-4-scout degradation as 06-20). FIX: the regex-double-fail branch now ALSO tries the Claude (Opus) parse fallback before routing to manual, gated on the SAME strong-bet-signal looks_like_bet (a unit token / @odds AND not no-bet framing -- so never on chatter) + the CLAUDE_TEXT_FALLBACK_TIPSTERS allow-list. saiyan_afl ADDED to that allow-list (it places via the PRICE-CHECKED AFL fan-out _place_afl_fanout; etr_nba stays EXCLUDED = blind no-price path). +test. (5) ROSTER SCRAPER FIX: the 06-21 04:30 AFL roster auto-refresh CORRUPTED roster_afl.json -- every name got a mojibake non-breaking space (Caleb\\xa0Daniel) -> exact-match resolution broke for ALL players. The pre-commit gate CAUGHT it (commit blocked, not pushed; the live bot loads the roster only at restart so it was never on the bad one). generate_afl_roster.py now forces resp.encoding=utf-8 + normalises \\xa0/garbage -> single space + a NAME-QUALITY guard (the player-COUNT guard alone passed the corrupted 802-name scrape). Roster re-normalised in place: 802 players, includes the now-present Hugo Hall-Kahan. Requires main.py restart. --- v5.81 (2026-06-21: Claude cost-tune + Trial Sniper track resolve (Wilson). (1) WEB-SEARCH RESOLVERS now use SONNET (CLAUDE_WEBSEARCH_MODEL=claude-sonnet-4-6), not Opus -- ~3x cheaper for the costly web-search path; the PARSE fallback stays OPUS (CLAUDE_FALLBACK_MODEL) for extraction accuracy. (2) web_search COST CAPS: tool max_uses=5 + pause-turn loop 6->3 (a single Opus+uncapped resolve cost ~$2 in testing -> now well under $1). (3) Track=None web-search resolution EXTENDED from Zak (SA-only, resolve_sa_track_today) to TRIAL SNIPER (general runner-name->track+race# lookup, resolve_racing_runner -- the OC-harness/racenet style: name is the anchor, corrects the race#). Flag renamed sa_track_claude_resolved -> track_claude_resolved (still DISABLES racing_placer saddle-only = NAME-or-manual, now both tipsters). (4) New Anthropic key. Live bot has spent $0 on Claude since v5.80 deploy (no Groq failures); the $2.29 was the v5.80 smoke test (~all from the one Opus web-search). Requires main.py restart. --- v5.80 (2026-06-21: CLAUDE API RECOVERY LAYER + 06-20 fixes (Wilson; built then reviewed by 5 opus xhigh subagents). THE BIG ONE: a PER-CALL CLAUDE FALLBACK (Opus 4.8, OFF by default behind CLAUDE_FALLBACK_ENABLED). Groq runs FIRST; Claude fires ONLY on a GENUINE parse failure and NEVER in addition (no Groq+Claude double-parse -- Claude fires only when Groq returned ZERO tips): (a) Shook/Groq-only TEXT on a looks-like-bet msg, (b) Eddie VISION 0-tips AFTER the v5.58 summary/recap suppress (so never on a no-bet/recap image -- the AusBets phantom-bet hole stays closed), (c) racing free-TEXT on a Groq CRASH only (never the by-design chatter drop). Every Claude-recovered tip re-enters the IDENTICAL placement/roster/floor/dedup gates -- no second betting path. Fixes the 06-20 Groq llama-4-scout SERVING DEGRADATION (15:50-19:20 emitted chain-of-thought / leaked Llama control tokens instead of JSON) that lost ~3 Shook HRRBI SGMs (~$1200) + 5 Eddie evening disposal images to manual. Claude WEB-SEARCH RESOLVERS (CLAUDE_WEBSEARCH_RESOLVE) for what the roster/catalog can't resolve: AFL player->current club (the just-drafted-as-a-Crow Hugo Hall-Kahan, absent from the stale roster) + Zak SA track-of-the-day (track=None) -- the resolver supplies ONLY the team/track; the existing bookie catalog + runner NAME+number match + odds floor still gate the bet, so a wrong/hallucinated resolve fails to price -> manual, NEVER a blind/wrong bet. resolve_racing_runner (racenet) BUILT for a later TT-ambiguity wire. TIP 62251 SILENT-DROP FIXED: under concurrent TT placement one get_sessions() momentarily read the shared pool empty for a single tip while siblings saw 17 -- now RE-FETCHES the pool (2x, recovers the false-negative -> 62251 would have placed) and routes a genuinely-empty drop to TIP TITANS MANUAL WITH THE SELECTION (was a bare CRITICAL that read like a global outage); both the text + image-racing paths. GROQ MODEL SPLIT: GROQ_TEXT_MODEL / GROQ_VISION_MODEL in .env (vision must stay multimodal; text can be a stronger text model) so they tune independently + swap mid-incident with no redeploy (default both = llama-4-scout = no behaviour change). AFL ROSTER AUTO-REFRESH every 2 days (TipBot-AFLRosterUpdate, mirrors the NBA cadence) + an empty/partial-scrape GUARD (generate_afl_roster.py refuses to overwrite a thin scrape -> exit 2, keeps the live roster) so fringe/mid-season players get covered. ACTIVATION: ANTHROPIC_API_KEY in .env + pip install anthropic + CLAUDE_FALLBACK_ENABLED=true (and CLAUDE_WEBSEARCH_RESOLVE=true for the resolvers); compiles + full no-op with them off. Requires main.py restart. --- v5.79 (2026-06-20: REROUTE OVERS-ONLY REVERT (Wilson). The v5.78 'all AFL disposals fan-out, over+under' redistribute reroute is reverted to OVERS ONLY (still any tipster — Saiyan or Eddie). Rationale: the disposals UNDER per-account cap is [124,99,74,50], FAR below the OVER/threshold cap [300,250,200,150] -- so the accounts that already placed an UNDER have almost no headroom to absorb a reroute (rebetting the leftover just re-rejects). Overs have the headroom; only overs reroute. An UNDER that doesn't fill first time is left as-is -> manual ('if it's passed the first reroute it is what it is'). `_is_afl_disposals_fanout` -> `_is_afl_disposals_over` (over-detection gate restored: selection over / endswith over / _is_threshold, never under). Money-safety unchanged. +tests (gate flipped to overs-only both tipsters). Requires main.py restart. --- v5.78 (2026-06-20: follow-on to v5.77 (Wilson). (1) Daniel 68723 AFL non-disposals caps lowered 400->300 + goals 1000->600 to MIRROR Wilson/Adam/Ryan -> ALL 4 SB sports accounts (+ Alex 65463) now FULLY equal-cap on AFL (completes the v5.77 parity; Daniel was the last 400/1000 outlier). (2) REDISTRIBUTE-TO-SUCCESSFUL reroute WIDENED from 'Eddie disposals OVERS only' to ALL AFL DISPOSALS fan-out -- both tipsters (Saiyan + Eddie) and BOTH directions (over + under): whatever the first fan-out round leaves UNFILLED (an account failed on low balance / suspended / proxy, or laddered down) gets ONE reroute onto the accounts that PLACED (even-split of the remainder, 100/90/80/70% ladder, stop on reject); if the reroute can't fill it either it's manual ('if it's passed the first reroute it is what it is'). Money-safe unchanged: capped at each account's 1/n share (no over-stake), reuses _fanout_place_account (ambiguous->stop, no double-stake), ONE pass. _is_eddie_disposals_over -> _is_afl_disposals_fanout; _afl_overs_redistribute_topup -> _afl_redistribute_topup; EDDIE_OVERS_REDISTRIBUTE -> AFL_DISPOSALS_REDISTRIBUTE. (3) 11pm DAILY REVIEW OVERHAULED (DAILY_REVIEW_PROMPT.md) to autonomously catch the AVOIDABLE-miss classes that slipped past prior reviews on 06-19 -- now 5 opus subagents at XHIGH effort with an explicit AVOIDABLE-MISS CHECKLIST: (A) surname->manual that was a vision TYPO of a roster player or a resolvable same-surname collision; (B) lopsided/stale fan-out split + failed-account stake that should reroute; (C) unfilled with an OPAQUE reason or placeable sessions skipped by do-not-bet config; (D) PER-ACCOUNT repeated-failure patterns (same error >=2x = systemic: code-540 suspended / 508 low-balance / TAB intercept / 403 proxy) -> config/ops fix; (E) misleading HB error labels. The review now SECOND-GUESSES every non-fill (BY-DESIGN vs AVOIDABLE vs GENUINE-FAULT) instead of accepting 'placed-or-alerted'. Ops/tooling (prompt) -- not a live-bot code change; (1)+(2) require main.py restart. +tests updated (gate widened to Saiyan+both-directions; base fan-out test pins redistribute OFF). --- v5.77 (2026-06-20: MULTI-FIX from the Wilson 06-19 review (reviewed by 5 independent opus subagents -- no over/double-stake, no wrong-player). (1) CAPS: Adam 65465 AFL non-disposals caps lowered 400->300 + goals 1000->600 to MIRROR Wilson 53522 -> the 4 SB sports accounts are equal-cap (disposals ladders were already identical). Daniel 68723 still 400/1000 -- FLAGGED to Wilson, not auto-changed. (2) MLB HRRBI SGM 4-way -> 3-WAY (Adam 65465/Wilson 53522/Daniel 68723 = $400/3 ~$133 ea); Ryan 102506 DROPPED from the SGM split (Sportsbet 'outcome is suspended code=540' on EVERY Ryan HRRBI SGM, 06-18 + 06-19); unfilled now spills to Alex 65463 THEN Ryan 102506 as 2+ HRRBI SINGLES (both SGM-disabled; Ryan given a player_stats ladder to mirror Alex). (3) NBA SGM drops Ryan -> Adam/Wilson/Daniel (matches AFL SGM). (4) Eddie bare-surname FUZZY fallback (difflib SequenceMatcher >=0.85, SCOPED to the in-play game roster, resolve only if a SINGLE player matches else manual) when the EXACT match misses a VISION TYPO -- 06-19 'D'Ambrossio' -> Massimo D'Ambrosio (scores 0.947 so 0.95 would MISS it; <4-char tokens skip). (5) Eddie surname COLLISION odds tie-break now resolves to the in-range candidate CLOSEST to the tipped odds (was: require EVERY candidate priced AND exactly-one-in-range -> bailed to manual when a same-surname sibling had no market; 06-19 Noah Anderson o28.5 disposals priced while defender Cody Anderson had none); equidistant top-2 -> manual; the 20% odds tol still gates eligibility. (6) Eddie BIG-bet (>2.5u) AFL fan-out split is now EVEN (intended/n + 10% decay ladder) -- retired the '$150 floor on the limited accounts + dump the remainder on the single highest-cap account' shape (06-19 Uwland concentrated $600 on Adam), obsolete now caps are equal. (7) Eddie AFL DISPOSALS-OVER redistribute-to-successful TOP-UP: when the fan-out leaves stake unfilled (an account failed -- e.g. Alex low balance -- or laddered down) AND others placed, re-split the WHOLE unfilled remainder EVENLY across the accounts that PLACED and top each up (100/90/80/70% ladder, stop on reject). Capped at each account's 1/n share (CANNOT exceed the unit) + reuses _fanout_place_account (ambiguous->stop, treated as committed = NO double-stake) + ONE pass. Disposals-OVER singles ONLY (unders/SGMs excluded -- harder liability). Gated EDDIE_OVERS_REDISTRIBUTE (kill-switch). (8) Racing UNFILLED alert now surfaces the in-range sessions that were SKIPPED (do-not-bet) + a below-floor count, instead of a blank '(no failure details)' (06-19 Minor Catastrophe: 13/18 below floor + 4 placeable accounts do-not-bet at Gloucester Park -> $1009 manual, unexplained). (9) Eddie image-channel TEXT gate: REMOVED the soft betting-VOCABULARY keywords (odds/tip/tips/back/lay/selection(s)/runner(s)/to win/the place/add) that pinged manual on CHATTER ('Just running the odds for the last bets', 'goal scorer pick ~8.30 AM', 'would've picked X last night'); TIME-guarded the decimal-odds pattern (8.30 am != odds); ADDED X+ ('25+ disposals') + over/under-LINE ('o30'/'over 30'/'under 23.5') selection patterns so a real text tip still pings -- instructions (scratch/cancel/late mail) + bet types (multi/sgm/...) kept. NOTE on (B): the HB 'sgm_bet_failed_unknown (code=508)' on a SINGLE = HyperBot's literal mislabel of an insufficient-balance reject (raise with HB). Requires main.py restart. +test_v577_disposals_overs_and_fuzzy, updated 5 existing tests. --- v5.76 (2026-06-19: AUDIT.JSONL TEST-POLLUTION LEAK CLOSED (Wilson 06-18 daily-review Issue 3). The v5.43 fix redirected ONLY _audit_tip() to a temp file under TIPBOT_TESTING, but the placement-path `tip_outcome` writers (7 `_log_jsonl(AUDIT_LOG, ...)` sites incl. the MLB/AFL SGM + fan-out outcomes) still hard-coded the production logs/audit.jsonl -> the unit suite leaked test rows (session_id 'x'/'C', bet_id null, empty event) into the real audit on EVERY run (~30 in the 06-18 ~01:29-01:51 deploy window; ~129 cumulatively; made the 06-18 audit mis-sum $9505/$2000-ambiguous vs the real $3285/$0). Fix: one call-time resolver `_audit_log_path()` (temp under TIPBOT_TESTING, else AUDIT_LOG) that EVERY audit writer now routes through -- _audit_tip refactored onto it too. Production path UNCHANGED (only diverts under TIPBOT_TESTING). Verified by an opus subagent: all 9 audit writers redirected, no other module writes audit.jsonl, the new test FAILS on a simulated revert. +3 assertions in test_audit_tip_test_isolation (resolver returns temp + a raw tip_outcome placement-write doesn't grow production). NOTE: the ~129 historical pollution rows already in logs/audit.jsonl are NOT auto-stripped (left for a manual filtered clean, like the v5.41 bet-ledger cutover). No production behaviour change (test-only redirect); main.py restart loads it. --- v5.75 (2026-06-18: EDDIE IMAGE WINNING-MARGIN -> PLACEABLE LINE (Wilson 06-17 review). Eddie's multi-bet AFL image was FULLY extracted (Groq got all 5 bets), but the 'Adelaide 40+' winning-margin leg was MISPARSED as 'Adelaide over 0.5 GOALS @ 4.50' (player_prop) -> goalscorer_threshold line=0.5 not carried -> manual; and even a correctly-typed `margin` market force-routed to manual (no placement path). Fix: (1) IMAGE_PROMPT_AFL now disambiguates a TEAM directly followed by 'N+' at longer odds ($3-$15) as a WINNING-MARGIN bet (market_type=margin, line=N, side=over) and NEVER a goals/player prop -- only a PLAYER name carries a goals/disposals prop. (2) _build_afl_tip_from_image CONVERTS margin -> a placeable -(N-0.5) LINE bet (Adelaide 40+ -> Adelaide -39.5; Wilson: '40+ winning = -39.5 alt line/handicap') so the bet is ATTEMPTED via the handicap matcher (±0.5 catalog match; a bookie miss still -> manual, never a blind/wrong line). The OTHER 'missed' bet (GWS u184.5) was NOT a bug: totals already snap within ±1.0 (_match_total_in_catalog) + a ±0.5 alt-line fallback, but u184.5 was 2.0 off SB's only line (u182.5) and u182.5 is a WORSE under -> correctly routed to manual with a BET UNFILLED alert (Eddie's own note: take u183.5 at Lads). Both misses DID fire manual alerts -- nothing was silently dropped. +3 margin regression assertions (test_image_afl_team_markets flipped from 'margin still manual'). Requires main.py restart. --- v5.74 (2026-06-17: MLB HRRBI 4-WAY EVEN SPLIT + %-LADDER (Wilson). The Shook 1+/2+ HRRBI multi now EVEN-SPLITS the $400 into 4 x $100 placed CONCURRENTLY across FOUR SGM accounts -- added Ryan 102506 (who can do multis + is now SB-limited) alongside Adam 65465 / Wilson 53522 / Daniel 68723 (was 3). Each account ladders DOWN 100/90/85/80% of its $100 stake on a bookie reject (MLB_HRRBI_LADDER_PCT; replaces the mlb.sgm liability bracket ladder for MLB only -- gated `if sport==mlb` in _place_sgm_fanout, AFL SGM unchanged). Any remainder UNFILLED after the 4-way SGM still SPILLS to Alex 65463 as a 2+ HRRBI SINGLE (kept as the spillover backstop). Config: MLB_SGM_SESSION_PRIORITY += 102506; MLB_HRRBI_SINGLE_SESSIONS stays 65463; new MLB_HRRBI_LADDER_PCT=1.0,0.9,0.85,0.8. Verified: 4x$100 when accepted, 4x$90 when $100 rejected ($40 -> Alex). Requires main.py restart. +1 test; updated the mlb.sgm priority test to 4 accounts. --- v5.73 (2026-06-17: RYAN SB SPORTS CAPS -> MIRROR WILSON 53522 (Wilson). Ryan Suwandy sportsbet 102506 is now bookie-LIMITED too, so its sports liability caps (nba/afl/mlb) were changed from the original 4.5x un-limited values (added v5.63/v5.65) to MIRROR Wilson 53522's standard limited Sportsbet caps -- keep 102506 in sync with 53522. EFFECT: the AFL singles fan-out capacity-weighted split (v5.65/66) now treats Ryan as an EQUAL-cap account -> EVEN split across the 5 sportsbet accounts instead of Ryan carrying the 4.5:1 share. sessions.yaml only; requires main.py restart to load. The v5.65/66 fan-out tests were rewritten to verify the weighting MATH with synthetic caps (so they no longer depend on a live account's caps) + assert the new live parity. --- v5.72 (2026-06-16: MLB ROSTER TEAM-OVERRIDE (Wilson; Matt Olson 06-16). The MLB resolve branch only inferred a team from the roster when NO team was stated ('if not team'), so a Groq-HALLUCINATED team (context bleed: a teamless 'Matt Olson 2+ HRRBI' inherited 'MIL' from the prior Brice Turang/MIL play + a standalone 'MIL' header in the 4-msg context) was trusted -> resolve_mlb_event('MIL') resolved the WRONG game (Milwaukee Brewers v Cleveland Guardians; Olson is an Atlanta Brave) -> Olson absent from that catalog -> NOT matched -> manual, $400 unfilled. Fix: the roster is now AUTHORITATIVE -- resolve_event runs roster.exact_match_player(player,'mlb') even when a team IS stated and OVERRIDES it on an exact full-name disagreement (mirrors the NBA team-override), turning 'MIL' -> 'Atlanta Braves'. EXACT full-name only (no fuzzy/bare-surname, the v5.33 hardening kept) + roster_mlb.json refreshed daily from the MLB Stats API (fresher than Groq), so it can only move a bet from the wrong game to the roster-correct game, never a wrong player; a typo/no-match falls back to the stated team (status quo). Plus an MLB-scoped Groq SYSTEM_PROMPT guard: do NOT copy a team from a different play/header in RECENT CONTEXT onto a later teamless player. +2 regression tests (and updated the v5.33 'stated team never overridden' test to the v5.72 override). --- v5.71 (2026-06-16: NOTIFIER READ-TIMEOUT DEDUP (Wilson). The v5.56 one-retry in notifier._send retried on ANY exception incl. requests.ReadTimeout -- but a READ timeout means the POST was TRANSMITTED and Telegram very likely already posted the message (only the HTTP response didn't return in time), so the retry re-POSTs the identical message and DELIVERS A SECOND COPY while the bot logs NOTIFY LOST. Caused the 06-16 ~19:53 Turang BET PLACED + Olson MANUAL double-sends during a Telegram slowness blip (placement itself was fine -- 4 distinct bet_ids, no double-bet). Fix: _send now (1) does NOT retry a ReadTimeout (logs 'DELIVERY-UNCERTAIN (read timeout)' instead of NOTIFY LOST -- the bot genuinely can't know if it landed); connect-level failures (ConnectTimeout/ConnectionError/DNS/refused = provably not sent) still get the one retry; (2) uses a (connect=5s, read=20s) timeout tuple so a slow-but-successful Telegram response is received rather than aborted at the old flat 10s. ReadTimeout imported at module level so the except resolves under a mocked requests in tests. +1 regression test. --- v5.70 (2026-06-15: BUG-HUNT ROUND 2 (9 fixes, 8 were REGRESSIONS from v5.69; report BUGHUNT_2026-06-15.md 'Round 2'). A second 12-agent pass (6 reviewing the v5.69 diff + 6 tail-hunters, adversarial-verified) found 18 confirmed, 13 regressions. Fixed: (R2#1, the worst) m11/m12 image conflict-key now NORMALISES period -- adding raw vision `period` to the key let over+under of the SAME full-game market escape the guard (one row 'full', the other null) and auto-place BOTH sides; period collapses to a full-game bucket + stat normalised. (R2#2/#6) M8 NBA disambiguation: a CONFIDENT full-name tip now resolves the top playable instead of false-manual when a similar same-surname sibling within the floor also plays; a BARE surname prefers the actual SURNAME match over a first-name collision ('Grant'->Jerami Grant not 'Grant Williams'). (R2#8) m16 now CLEARS a stale _is_threshold on an 'under' leg (was only skipping NEW promotion -> a model-contradictory {under, is_threshold:true} leg still placed as the over side). (R2#3) m18 0-tip ping suppression gated on a CONCRETE selection (_image_text_selection_pattern/urgent) not the broad actionable keywords ('tips'/'odds'/'selections' appear in real recaps -> re-spam). (R2#5/#7) m9 regex-tipster parse-fail alert now requires a unit/@-odds token AND not-no-bet-framing (was alerting on no-bet/recap chatter). (R2#17) i4 no-bet-framing regex tightened -- the {0,3}-word filler matched 'no bets ON X today, but <real bet>' and suppressed the real bet. (R2#10/#11) M3 bookie-alias added to the v4 SINGLES reconcile too (was only fan-out + SGM). (R2#13-15) m4 racing reconcile-placed now calls _pace_record_placed. (R2#4) bet_record_feed carries forward a settled Result that ages past the 14d window instead of reverting to blank. +autostart deprecation note. TAB 100003 skip-list deferred (needs 403 confirmation). +7 regression tests. --- v5.69 (2026-06-15: FULL-REPO BUG-HUNT (31 fixes; 10-agent end-to-end audit + adversarial verify, report BUGHUNT_2026-06-15.md). MAJORS: (M6) place_tip alert_only short-circuit made UNCONDITIONAL -- it was `alert_only and not is_sgm`, so a no-unit/no-bet AusBets/Kev msg promoted to an NBA SGM auto-placed past the v5.52 money-gate; (M3) reconcile now passes the BOOKIE-aliased event (_bookie_event) into decide_ambiguous in _reconcile_fanout_ambiguous + _place_sgm_v4 -- tip.event (Squiggle 'Greater Western Sydney') never matched pending_bets ('GWS Giants') so a landed AFL bet read as not-found -> manual re-bet (double); (M1) reconcile-CONFIRMED-placed now converts to a real SUCCESS (ledger/summary row) in the SEQUENTIAL v4 singles + _place_sgm_v4 paths (the v5.55 fan-out fix was never back-ported -> debited-but-never-ledgered + false 'MAY have placed' critical); (M2) SGM boost->no-boost retry now SKIPS when the boost attempt was slow/ambiguous (maybe-landed) -- closed an Erasmus double-stake hole (latent, boost off) in _place_sgm_v4 + legacy _place_sgm; (M8) NBA teamless disambiguation rebuilt -- generates candidates from the ORIGINAL bare surname (not the resolve_player_name-collapsed name, which biased scores) and routes 2+ same-surname players-both-playing-within-floor to MANUAL instead of an arbitrary first-pick; (M7) kev/ausbets regex parsers RESET _last_player/_last_stat per message (cross-message carry-forward leaked a previous game's player onto a follow-up tip); (M5) racing bookies missing from mbl_caps.yaml no longer place UNCAPPED -- added neds/hotbet/palmerbet/betright + _resolve_liability_cap fail-safe to a conservative country MBL; (M4) racing dedup _release_bet uses the SNAPSHOTTED claim fp so place_racing_tip's undated date-mutation can't leak a claim + suppress a valid re-send. MINORS: m4 racing reconcile-placed -> result['placed'] (ledger); m5 SGM fan-out splits over PLACEABLE accounts only (no permanent under-fill); m6 AFL fan-out OVER->threshold market flip moved BEFORE the weighted split; m7 h2h price-change retry flags a below-floor fill to Maintenance; m8 auto-alt restores leg.line on success; m9 regex-tipster parse-fail now alerts; m10 Shook 30s cooldown is content-aware (distinct prop within 30s no longer dropped); m11/m12 AFL image conflict key includes stat + resolves totals on event; m13 AFL SGM line match is exact-only (no silent 30+->29+ snap); m14 _find_prop_id direction match is exact (no 'Grover'-contains-'over' leak); m16 integer 'under' SGM leg no longer flipped to an over threshold; m17 v4 alt-chain sizes on the ALT leg's market/no-odds; m18 0-tip image summary-suppression skipped when the caption is actionable; m2 tip_parser Claude path got the v5.23 stat-guard + units_explicit; m1 replay_image_parse tuple unpack. INFO: i2 MLB HRRBI writes ONE consolidated audit row (incl Alex); i3 get_transactions retries the idempotent read; i4 no-bet-framing regex relaxed + applied to all UNITS_REQUIRED_TIPSTERS; i5 bet_ledger dedup seeds from CSV (survives restart); i1 _readme stamp. +13 regression tests. --- v5.68 (2026-06-15: FAN-OUT ONE-RETRY ON TRANSIENT PRE-PLACEMENT REJECT (Wilson). A proxy 403 / auth / network reject in the AFL singles + SGM fan-out means the bet was NEVER submitted to the bookie (gated by _is_definitely_pre_placement -- the narrow provably-not-placed set: forbidden/403/401/400/bad request/stale_command), so it now RETRIES the SAME rung ONCE (AFL_FANOUT_RETRY_DELAY_SEC=2s) before abandoning -- ZERO double-stake risk. Evidence: Alex 65463's Bailey $150 on 06-14 was a one-off proxy 403 that placed fine 12 min later (Alex went 7/9 on the day, placing 4 MORE bets after the 403) -> a retry would have filled it instead of dropping to Manual. Stake-rejects still ladder DOWN; AMBIGUOUS/maybe-landed still STOPS and is NEVER retried (Erasmus safety). Gated AFL_FANOUT_PREPLACEMENT_RETRY (default true). --- v5.67 (2026-06-15: EDDIE IMAGE PARSE FIXES (Wilson 06-14 day-review). 1) VISION MULTI-BET: the AFL image prompt (IMAGE_PROMPT_AFL) said 'Most images contain exactly ONE bet' -> Groq UNDER-extracted Eddie's multi-bet images (Ashcroft o26.5 3u + 30+ 0.5u -> ONE merged tip with the 26.5 line but 0.5u units, 30+ dropped; Greene 2+/3+/4+ -> 1 tip). Prompt rewritten: extract EVERY selection as its OWN tip, and the SAME player on multiple lines/thresholds -> a SEPARATE tip per line, units/odds read from THAT line only, NEVER merge. Validate live via replay_image_parse.py against the real images. 2) EDDIE SURNAME TIER-2 FALLBACK: _resolve_eddie_surname_to_player now tries the 2h-window scope (Tier 1) THEN falls back to ALL of TODAY'S fixtures' rosters regardless of time (new resolver.afl_games_on_date) -- Greene tipped 10:13 for a game >2h away went to manual; now resolves if the surname is unique across today's fixtures (ambiguity still -> manual, odds tie-break preserved). NOTE: Bailey/Alex 65463 failure on 06-14 was a PROXY 403 (HyperBot-side, not tipbot) -- the v5.66 Eddie big-bet $150/$600 decay fired correctly. --- v5.66 (2026-06-14: AFL FAN-OUT SPLIT SHAPES (Wilson). Refines v5.65. NORMAL (Saiyan any size; Eddie <=2.5u): the capacity-weighted split is CLAMPED so the high-cap acct (Ryan 102506) carries at most AFL_FANOUT_RATIO_CAP=4x the smallest -> 4:1:1:1:1 = Saiyan @600 $75 ea + $300 Ryan; Eddie @2.5u=1000 $125 ea + $500 Ryan (scales with unit). EDDIE BIG BETS (units > EDDIE_FANOUT_BIG_UNITS=2.5): each of the 4 LIMITED accts starts at EDDIE_BIG_LIMITED_STAKE=$150 (~their late-game disposals liability), Ryan takes the REMAINDER, EVERY acct ladders DOWN 10%/step (EDDIE_FANOUT_DECAY=0.9) until it places, unfilled -> Manual. New pure _afl_fanout_targets() (per-acct targets + ladder mode: yaml-brackets or decay), unit-tested; FAIL-SAFE to even split if any cap non-numeric. Budget still capped at unit. Saiyan has no big-bet escalation (always 4:1). --- v5.65 (2026-06-14: AFL SINGLES FAN-OUT CAPACITY-WEIGHTED SPLIT (Wilson). _place_afl_fanout (Saiyan + Eddie AFL singles) now splits the unit PROPORTIONAL to each account's liability cap (top bracket) instead of even 1/n -> Ryan 102506 (4.5x caps) carries 4.5:1:1:1:1 vs the 4 limited sportsbet accounts. A $1000 (2.5u) tip now fills FULLY (~$118 ea on the 4 limited which fits their ~$138 disposals cap; ~$529 on Ryan under his ~$620) vs even-split's ~$752 + $248->Manual. Still budget-capped at the unit + ladders DOWN on reject. New _afl_fanout_weights() (pure, unit-tested); FAIL-SAFE to even split if any acct lacks a clean numeric cap. Gated AFL_FANOUT_WEIGHTED (default true). SGM fan-out UNCHANGED (even-split across the 3 equal-cap accts = same result); Eddie unit stays $400. --- v5.64 (2026-06-14: SAIYAN SGM UNIT 600->750 (Wilson). Singles/disposals stay at SAIYAN_UNIT_SIZE=600 (hard unit); SGMs now use SAIYAN_SGM_UNIT_SIZE=750 -- the concurrent SGM fan-out EVEN-SPLITS the unit across the 3 SGM accounts (Adam 65465/Wilson 53522/Daniel 68723) so 750 -> $250 stake each (was $200 = 600/3). New _apply_saiyan_sgm_unit() mirrors _apply_mlb_flat_stake (overrides tip.unit_size in place, units preserved; runs after the image test-stake override). Ryan 102506 REMOVED from AFL_SGM (kept in AFL/NBA singles only -- 'dont add Ryan to SGM'). NOTE: the per-account afl.sgm liability ladder [400,300,200] still caps a high-combined-odds SGM below $250 (binds above ~2.6 combined). Eddie unchanged -- already does the concurrent ladder fan-out (even-split = units respected, ladders down on reject, never overstakes). --- v5.63 (2026-06-14: NEW ACCOUNTS WIRED + low-success diagnosis (Wilson). 1) Ryan Suwandy sportsbet (102506) ADDED to sessions.yaml SPORTS-ONLY, caps = 4.5x the standard sportsbet sports caps (65465 template; fresh un-limited acct), appended at the BACK of AFL/NBA singles + AFL/NBA SGM priority lists (NOT MLB SGM -- that's the bespoke Shook HRRBI 3-acct play); NO racing block + NOT in RACING_SESSION_PRIORITY. 2) Alex Liu neds (100006/ALPunts) appended to the racing tail (was set up + active but not placeable). 3) Diagnosed the 3-day low-success accounts: 78280 TAB + 100003 TAB = HyperBot 'Intercept handling failed'/SidecarRequestError (TAB sidecar, already -> HyperBot ops); 73359 bet365 = 'Bet placement failed' (flaky bet365, same family as the pulled 53523/73357); 100004 ladbrokes = NOT a fault, odds-guard correctly rejecting price-below-target (7.5 vs 8.55 etc), healthy; 100005 betr = 'Odds changed 2103' betslip re-accept gap (betr support unverified) + one transient insufficient-funds. 4/5 are HyperBot bookie-integration issues, not tipbot/limits/funding. .env priority-list edits need main.py restart. --- v5.62 (2026-06-14: SPORTSBET HARNESS CAP TUNE (53522 + 71275) -- follow-on to the v5.61 limits analysis. sessions.yaml only. 53522 (Wilson): harness PLACE ceiling ~$500 + WIN ~$750 -> lowered Albion Park/Marburg place 1000->500 and the Saturday overrides (win 1500->750, place 1000->500; the bookie does NOT allow more on Sat for this limited account, it caused 61997 05-30 + the 06-13 place rejections). 71275 (Jocelyn, more limited): harness PLACE ceiling ~$160-200 + WIN ~$400-500 -> all place 400->200, Gloucester Park win 1000->500. Thoroughbreds untouched on both (never limited). Caps are the bot's OWN risk cap; tuning to the bookie's real ceiling stops rejection churn + (with v5.61) MBL pings, not placed volume. Requires main.py restart to load. --- v5.61 (NOTIFICATION-NOISE REDUCTION, Wilson day-review). Goal: fewer alerts. 1) SESSION-HEALTH SILENT BACKUP: main.py's in-process _session_watchdog stamps logs/main_watchdog_heartbeat.txt every cycle (+at startup); the scheduled check_session_health.py now DEFERS entirely while that heartbeat is fresh (<800s) and only takes over when stale (main.py dead / watchdog stalled) -- kills the double-alerting (a real drop used to page BOTH monitors ~15min apart). 2) FOREIGN ACCOUNTS NEVER ALERT: check_session_health filters the shared HyperBot session list to OWNED ids (sessions.yaml) + prunes foreign from its state, so other PCs' accounts dropping/recovering stop pinging (was hitting Maintenance at 14:40/23:40/00:20). Fail-safe: yaml glitch -> track all. 3) MASS-DROP/RECOVERY BATCHING: check_session_health sends AT MOST one Critical + one Info per poll (was one Telegram PER session -- the 00:10 mass-drop fired 15 Criticals + 15 recoveries). 4) XWATCHER CRASH TRIMMED: x_watcher._alert strips Playwright's multi-KB 'Browser logs:'/traceback tail + caps to 280 chars (full detail still in x_watcher.log). 5) RACING BOOKIE STAKE-CAP (MBL) routing mirrors sports v5.52: Critical ONLY when nothing placed; when the ladder still landed a smaller bet it's suppressed (LADDER ACTIVITY on Maintenance already covers it) or routed to Maintenance -- stops the double-ping on tips 62179/62194. 6) Watchdog +60s recheck only speaks up when something RECOVERED (all-still-down recheck was noise; 15-min Critical covers it). LIMITS ANALYSIS (Wilson): sportsbet 53522 place@11 limited to ~$50 (cap allowed $100); 71275 place@9.5 limited to ~$19 (cap allowed $47) -- recommend lowering those racing place caps.)"
