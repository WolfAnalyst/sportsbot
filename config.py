...
TIPBOT_VERSION = "v5.60 (2026-06-13: FB TENNIS IGNORED + AFL SGM RESOLVES OFF ANY LEG (Wilson day-review). 1) IGNORE_TITANS (env TIPTITANS_IGNORE_TITANS, default FB): an ignored-titan tip is dropped silently with a log line, NO manual ping (FB tips tennis + never auto-placed since AUTO_PLACE_TITANS={OC}; the manual alerts were noise). 2) resolve_event AFL now falls back to any leg team (tip.all_teams) when primary_team doesnt resolve -- saiyan SGM LDU 22+/Harry Sheezel 24+ failed No fixture found for LDU (Groq didnt expand LDU->Luke Davies-Uniacke so leg0 had no team) though leg1 Sheezel resolved North Melbourne; should have auto-placed. Singles unaffected. Day-review: 15 manual/failed items, 13 by-design/bookie-side, these 2 fixed; Rowbottom integer-line-vs-halfline-ladder DEFERRED (agents threshold-market fix was wrong), tab:78280 Intercept errors -> HyperBot ops.)"
