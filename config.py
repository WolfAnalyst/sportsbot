...
TIPBOT_VERSION = "v5.5 (2026-06-03: symmetric min-odds FLOOR price guard for sports. _execute_bet already routed to manual when live catalog odds were >1.25x ABOVE tipped (ceiling=wrong market); added the symmetric FLOOR (_below_odds_floor): live >10% BELOW tipped (live < tipped x0.9) -> manual with a clear 'price floor' reason instead of placing at a much shorter price. Pairs with the existing target_odds=tippedx0.9 bookie-side floor but routes cleanly to manual. All sports; only fires when live odds were captured (catalog match); missing price never blocks. Racing already had floor+ceiling->manual in price_shop. Builds on v5.4 (AFL totals at exact alt line via pick_own_total).)"
