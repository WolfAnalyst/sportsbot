...
TIPBOT_VERSION = "v5.4 (2026-06-03: AFL TOTALS place at the exact alt line. Completes v5.3 after verifying the live Sportsbet AFL catalog (Adelaide v Geelong via /v3/price): total_points carries only the MAIN line (~166.5), line carries the team handicap (Geelong -8.5 @ 1.9, places), pick_own_total/pick_own_line carry every-0.5 alt lines. New _match_total_in_catalog resolves total_points (+-1.0) then pick_own_total (+-0.5, baked line -> proposition_id), mirroring _match_handicap_in_catalog; _execute_bet's total_points branch now uses it so Eddie's 172.5 (vs 166.5 main) places at the exact alt line instead of going to manual. Catalog miss -> manual; odds ceiling still guards skewed alt prices; $1-gated. Builds on v5.3 (ante-post date carry, AFL totals/lines, over-parse guard).)"
