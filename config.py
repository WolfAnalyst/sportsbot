...
TIPBOT_VERSION = "v5.51 (2026-06-10: THOROUGHBREDS LIABILITY CAP WIRING (Wilson). _resolve_liability_cap's `discipline` arg was never passed at its only call site (place_racing_tip), so racing.thoroughbreds.{win,place} was DEAD -- every (R) Zak/Trial tip fell through to the harness default:mbl path (capped at the bookie MBL / 'no-cap', NOT the configured $1000/$500; Trial's Red Sentinel on 06-10 sized at $500 mbl / hotbet no-cap instead of $1000). FIX: place_racing_tip derives discipline from race_type ((R)->thoroughbred, (H)->harness, (G)->greyhound) and passes it; ONLY (R) changes behaviour, harness/greyhound/None take the existing track/MBL path unchanged. All 19 racing-priority sessions carry a thoroughbreds cap so none get fail-safe-skipped. Caps kept win 1000 / place 500 (Wilson). test_racing_thoroughbred_cap_wired. racing_placer.py tipbot-only. PREV v5.50: RACING CONFIRMED-NOT-PLACED SPILL (Wilson). When a racing leg is CONFIRMED-not-placed by reconcile (a successful /api/pending_bets poll proves the bet is absent — e.g. today's bet365 websocket-fail KOOJAN R6 win $600 + tabtouch bet-limited), the unplaced amount is now RE-SHOPPED onto the NEXT eligible bookie instead of routed to manual/unfilled — while STILL firing a critical alert (the bookie rejected for some reason). Previously such a leg DEBITED `remaining` (stopping the spill) and went to manual, so ~$640 of recoverable racing stake went unfilled on 2026-06-09 even though other bookies had capacity. IMPL (racing_placer.py): on decide_ambiguous action=='spill' (returned ONLY on a POSITIVE not-placed confirmation — never on uncertainty [reconcile_failed→'conservative'] or a landed bet ['placed']), blocklist the bookie + DO NOT debit `remaining`, so the price-shop loop re-places it on the next eligible bookie at its shopped price + the max(quote,tipster)×0.9 target-odds floor; record result['spilled_not_placed'] (INFORMATIONAL — NOT added to unfilled; the amount is either re-placed [result['placed']] or falls through to unfilled→manual if no bookie can take it). CARVE-OUT (the documented Tier-2 double-bet trap): spill_enabled = config.RACING_RECONCILE_SPILL (default true) AND _racing_spill_safe(bookie) = bookie not in config.RACING_NO_SPILL_BOOKIES (default 'pointsbet') — NEVER spill a slow-landing/trader-review book whose real land can arrive AFTER the reconcile poll window (a >window land reads 'not found' → spill → DOUBLE bet). Requires RECONCILE_AMBIGUOUS on (the master poll switch; on in prod). Crit alert for spilled legs fires in tiptitans_processor.py (both the text + image-racing paths). test_racing_confirmed_not_placed_spill (carve-out + spill/not_placed/placed/conservative). 2-lens adversarial verify (double-bet safety + flow/accounting). racing_placer.py + tiptitans_processor.py are tipbot-only (NOT synced; the fork is sports-only, no racing). config.py shared → synced (the racing flags are inert in the fork). PRIOR v5.49: DEDUP SURVIVES PLACEMENT MUTATION + racing caps (Wilson). (1) DEDUP FIX: the tip dedup CHECKED _is_duplicate BEFORE place_tip but REGISTERED the fingerprint AFTER — and place_tip MUTATES legs (the catalog match rewrites a leg selection, e.g. 'James Wood Over'->'James Wood'), so the registered post-mutation fingerprint never matched the pre-mutation fingerprint a re-send checks -> the dedup was DEFEATED and a re-sent Shook MLB HRRBI re-placed an extra $100 (2026-06-09 James Wood 2+ HRRBI: Shook re-posted 67s later re-including James Wood + adding CJ Abrams; the duplicate SGM was blocked by sportsbet code-538 but Alex's $100 single slipped through). FIX: capture the fingerprint BEFORE place_tip and register that exact string at BOTH dedup sites (text pipeline + image pipeline); _register_tip_fingerprint(tip, fp=None) gains an optional precomputed fp (None recomputes, for legacy callers). So check==register regardless of placement mutation — fixes the dedup for ALL tipsters, not just MLB. test_dedup_survives_placement_mutation (proves the mutation changes the fp AND that registering the pre-place fp catches the re-send); 2-lens adversarial verify. main.py shared -> synced. (2) RACING CAPS (sessions.yaml, tipbot-only, NOT synced): 53522 (Wilson Sportsbet) Gloucester Park harness WIN 1000->750 (today's stake-too-high reject); 71275 (Jocelyn Sportsbet) Gloucester Park harness PLACE 400->200 (it laddered $80->$60->$40 on MBL violations placing VINTAGE BLU; decreased per Wilson). FOLLOW-UP QUEUED (separate, RISKY): spill a CONFIRMED-NOT-PLACED racing leg (bet365/tabtouch) to the NEXT bookie via re-shop instead of routing to manual/unfilled, keeping the critical alert — this is the reconciliation Tier-2 spill with the documented double-bet carve-out (never spill a slow-landing/over-MBL bet). PRIOR v5.48: UNDATED RACING TIP -> NEXT THOROUGHBRED MEETING AT TRACK (Wilson). Zak Trussell posts the NEXT DAY's card the night before and the screenshot's date isn't carried into the vision output, so _img_parse_racing_date returned None and racing_placer DEFAULTED TO TODAY. On 2026-06-09 Mount Gambier was a HARNESS meeting while Zak's tips were the 06-10 THOROUGHBRED (R) card, so the (R) shop for 'today' got race_type_mismatch (sportsbet lists it as (H)) / 'not in catalog' on every book and all 6 tips (~$2700) failed/manual (the lone bet365 quote was a STALE cross-session (R) cache that failed at confirmbet 'stakes_above_max_stake' + tripped the circuit-breaker, cascading the R8 tips to manual). VERIFIED EMPIRICALLY: a $1 probe with date=2026-06-10 priced + PLACED 5/6 on sportsbet/tab/tabtouch (only Last Dream, a 61.0 longshot, missed on a stake-cap/session-not-ready, not the date). bet365 'does not support future-dated race markets' (same-day only); neds/ladbrokes DO carry the venue (both priced today's harness) but open ante-post LATER, so they catch up on race day. FIX (racing_placer.py, tipbot-only, NOT synced to the fork): new PURE _next_meeting_date(has_meeting_fn, base_date, max_days) (ascending probe today+1..+N, first hit, else None) + new config RACING_UNDATED_LOOKAHEAD_DAYS (default 3, env-overridable, 0 disables) + a block in place_racing_tip that, when a tip is UNDATED and today's (R) shop returned NOTHING (after the existing transient-miss retry), rolls the date forward up to 3 days to the first day the track has a (R) meeting and uses that day's cached shop result (read-only price-checks, no double-bet). Zak STAYS hardcoded thoroughbred — only the DATE is resolved, never the discipline (Wilson); the runner/saddle match in the price-check guards a same-numbered wrong-day race; a dated tip is unaffected; today is used when today HAS the (R) meeting. test_next_meeting_date; 2-lens adversarial verify. PRIOR v5.47: SEQUENTIAL-SGM AUDIT GAP — _place_sgm_v4 now writes a tip_outcome to audit.jsonl (Wilson). The SEQUENTIAL SGM path (used by NBA SGMs per the v5.38 dispatch) wrote NO tip_outcome at all — only the CONCURRENT _place_sgm_fanout did (its tail ~L7837) — so a placed sequential SGM was silently absent from the audit trail (2026-06-09 KAT u20.5 / Brunson o21.5 $400 @1.80 bet_id 6860622106 placed fine + recorded in bets_placed.csv, but no audit.jsonl row; only 3 of 4 NBA tip_outcomes logged that day). FIX: new PURE module-level helper _sgm_outcome_record(tip, intended_stake, placed_results, remaining_stake, orchestrated) builds the record (placed_stake = sum of placed, unfilled = max(0,remaining), fanout='sgm_sequential', placements w/ bet_ids); a new PER-CALL once-guard _sgm_outcome_logged + the existing universal terminal hook _emit_sgm_aux_alerts (called at EVERY _place_sgm_v4 return — early full-fill AND post-loop partial/exhausted) writes it exactly once, NON-orchestrated only (MLB orchestrated outcomes are owned by the orchestrator/fan-out; no double-write — _place_sgm_fanout is a SEPARATE fn with its own flag + its own ~L7837 write). Best-effort: the _log_jsonl is try/except'd and the helper is pure, so an audit write can NEVER break a placement (audit.jsonl is observational, NOT the money ledger — the bet record feed reads bets_placed.csv). Pure helper unit-tested (test_sgm_v4_audit_outcome: fanout/sum/unfilled-clamp/bet_id/partial-fill). main.py shared -> synced. Adversarially verified. PRIOR v5.46: MONITORING AUTOCHECKS — X-watcher liveness heartbeat + session-health autocheck (Wilson). Closes the silent-failure gap exposed when TAB's expired proxy killed both TAB sessions into terminal failure_hold and they sat DEAD ~33h on 2026-06-07 with NO alert (the HyperBot heartbeat only proves the engine PROCESS is up; the XWatcher watchdog only covers the Twitter watcher; check_sessions.py is a manual diagnostic) — nothing watched whether a bookie SESSION was actually alive. TWO additions, both tipbot-only, NO main.py / placement-path change (so NO Main restart needed — only XWatcher restarted for the heartbeat). (1) X-WATCHER LIVENESS HEARTBEAT: x_watcher.py now writes logs/x_watcher_heartbeat.txt every poll-loop iteration (the watcher's log only writes on a forward/seed/error, so a HUNG-but-alive watcher emits NOTHING for hours — the 2026-06-09 case: pid up, zero output ~23h, the existence-only watchdog blind to it; verified the watcher was actually fine that day, but the gap is real). xwatcher_watchdog.ps1 now ALSO restarts the watcher when the heartbeat goes STALE (>11min, comfortably above x_watcher's 300s hard back-off so a legitimate auth/fetch back-off never false-trips) — kills the hung tree + relaunches; a missing heartbeat file (just-started / pre-heartbeat build) is left alone. (2) SESSION-HEALTH AUTOCHECK: new scheduled task TipBot-SessionHealth (every 10min — daily trigger + 10min repetition, the proven watchdog pattern; register_session_health_task.ps1 + run_session_health.ps1 + check_session_health.py) queries /api/session_ids READ-ONLY via HyperBotClient.get_sessions_or_none(), learns the EXPECTED session set as a high-water-mark of every session seen active (persisted logs/session_health_state.json, self-configuring — no hardcoded account list to drift), and notifier.notify_critical()'s when an expected session goes MISSING or active=False for >= SESSION_HEALTH_FAIL_STREAK (default 2 ~= 20min) consecutive checks, with a recovery note when it returns. Anti-spam: consecutive-streak + already-alerted once-guard (the x_watcher learned the 44-alerts/day lesson); a never-alerted blip that recovers stays silent; SESSION_HEALTH_FORGET_SEC (default 7d) drops a long-retired account so it can't alert forever; an API outage (get_sessions None) SKIPS the cycle (no false alarm). KNOWN LIMITATION (documented): keys on the API active flag + list presence — catches failure_hold/unregister/inactive/dropped (the dominant + the TAB case) but NOT the rarer 'API reports active but functionally dead' (a future per-account log-freshness cross-check). Core evaluate() is a PURE state-machine, unit-tested in isolation (test_session_health_evaluate: down-after-streak / recover / no-spam / active=False / blip-no-recovery / forget). config.py only bumps the version (the new logic is in tipbot-only scripts) so the fork sync is a no-op-for-behaviour. FOLLOW-UPS QUEUED (separate change, need a Main restart + money-path adversarial verification): (a) SGM tip_outcome AUDIT GAP — a fully-filled SGM early-returns via _accumulate_and_check_done -> _emit_sgm_aux_alerts -> return BEFORE the _place_sgm_v4 tail tip_outcome write (line ~7836), so audit.jsonl misses it (2026-06-09 KAT/Brunson $400 bet_id 6860622106 placed fine + in the ledger, but no audit record); fix = move the tip_outcome into the once-guarded _emit_sgm_aux_alerts universal terminal hook (mirrors the v5.36 summary fix); (b) TAB ABSENT FROM RACING — both TAB sessions (71265/78280) are logged-in + in RACING_SESSION_PRIORITY with matching live IDs yet excluded from every racing PRICE SHOP INPUT today (and since ~06-07 10:56), ~$8k idle on racing; runtime exclusion (likely the racing_placer circuit-breaker stuck cooled / racing strategy not attaching), root-cause diagnosis in flight. PRIOR v5.45: BET RECORD FEED — generation ENABLED for a test run + clean-wipe cutover + 403-session skip (Wilson). GENERATION is now ON (.env BET_RECORD_FEED_ENABLED+_AUTORESULT=true) so the feed CSV auto-populates with Win/Loss/Void over the next few days for Wilson to eyeball — but the bettrack MERGE stays OFF (MERGE_TIPBOT_FEED in bettrack config_local.py, a SEPARATE gate); these are two independent gates so the dashboard is untouched while testing. New scheduled task TipBot-BetRecordFeed (gen_bet_record_feed.ps1 + register_bet_record_feed_task.ps1) regenerates the OneDrive CSV every 2h (daily trigger + 2h time-repetition, the proven-reliable watchdog pattern). LIVE-CONFIRMED 2026-06-08: today's 68 placements all auto-resulted 36 Win/29 Loss/3 Return, net +$4612.23 (the Saiyan pushed-leg SGM correctly -> Return/$0). NEW BET_RECORD_FEED_SINCE (YYYY-MM-DD, default empty=all) = the CLEAN-WIPE cutover: at switchover Wilson sets it to the cutover date + flips MERGE_TIPBOT_FEED on, so the feed emits ONLY post-cutover bets and there are no dupes with the tipbot bets he typed manually pre-switchover (ISO dates sort lexically; filter in generate()). NEW BET_RECORD_FEED_SKIP_SESSIONS (default 71265,78280,76341) = sessions whose bookie 403s /v3/transactions so auto-result skips them (no error spam / wasted polls; their bets stay blank = manual-result) — CONFIRMED 2026-06-08 the 403 sessions are TAB Wilson 71265 + TAB Daniel 78280 + Sophie PointsBet 76341 (Soup-side; TAB/PointsBet don't expose transactions). generate() now also writes atomically (tmp+replace) + is fully try/except-guarded + warns-not-clobbers on a duplicate bet_id. test_bet_record_feed_cutover. main.py + the live placement path STILL untouched (the generator is its own process via the task; the bot never opens the .xlsx). config.py shared -> synced (flags inert in the fork); bet_record_feed.py + the ps1 ops scripts are tipbot-only. PRIOR v5.44: BET RECORD FEED — BUILT BUT GATED OFF (Wilson). Export tipbot's placed-bet ledger (logs/bets_placed.csv) into ONE row per placement in Wilson's '2025-26 Bet Record' workbook conventions, so his bettrack dashboard merges tipbot bets with his manual record. NEW bet_record_feed.py: pure transforms -> Tipper (Cownie=OC + his existing names Saiyan/Eddie/Shook/ETR/Kev/Aus/Zak Trussell/TrialSniper/EasyMoneyAFL), Sport (AFL/NBA/MLB; racing -> '<STATE> <Harness|Thoroughbreds|Greyhounds>' with discipline-by-tipster [OC=Harness, Zak/Trial=Thoroughbreds] + a track->state map [blank state on a track miss, Wilson fills]), Team (player's team via roster / Multi for SGM / the meeting for racing / Totals / Exotics), Bet Name (his wording: '<Player> OVER/UNDER <line> <Stat>', '<Team> +<line> HC', '<Event> OVER/UNDER <line>', '<Runner> Win|Place'; SGMs list the legs), User (account owner's first name), Stated=Actual=filled odds (no Betfair commission), Stake(U)=$/100, dedup by bet_id. AUTO-RESULT: a new hyperbot_client.get_transactions(session_id) (POST /v3/transactions, async, READ-ONLY) returns SETTLED bets {id(=bookie bet_id, matches ledger bet_id), status(won/lost/void), pnl(net $)}; fetch_results joins by bet_id and fills Result (won->Win/lost->Loss/void->Void; UNKNOWN status -> left blank, never guessed) + Net Change (= pnl verbatim). bettrack server.py: NEW parse_feed_csv() + a GATED merge in load_data() (MERGE_TIPBOT_FEED default False) that appends the feed rows alongside the manual Bet Record, applying the SAME settled-only result filter so unsettled tipbot bets don't pollute the P&L; fully guarded (a feed problem can never break the manual Excel load); the feed-CSV mtime is folded into the cache key so new auto-resulted bets refresh without an Excel edit. KEY SAFETY: the bot NEVER opens/writes the .xlsx — it only writes the CSV — so it can't lock/corrupt the open workbook; the whole feature is INERT until Wilson flips BET_RECORD_FEED_ENABLED + MERGE_TIPBOT_FEED (main.py + the live placement path are UNCHANGED — nothing imports/calls the generator). Verified on the real 216-row ledger (preview): Cownie/QLD+WA Harness, Zak/SA Thoroughbreds, Trial/NSW+VIC Thoroughbreds, player-team resolution, SGM legs, Eddie total->Totals. tests test_bet_record_feed. ENABLE STEPS (later): set BET_RECORD_FEED_ENABLED=true (+ _AUTORESULT=true) in tipbot .env + wire a regeneration (a scheduled task running `python bet_record_feed.py` every N min, like TipBot-MLBRosterUpdate); set MERGE_TIPBOT_FEED=True in bettrack config_local.py. NOTE the auto-result money figures (pnl) should get a 2-min live /v3/transactions confirm before trusting them. PRIOR v5.43: EDDIE NO-UNITS FALLBACK — liability cap + bet-log flag (Wilson, refining v5.42). (1) BOOKIE-LIABILITY CAP: the no-units fallback already capped total STAKE at $1000; now it ALSO caps total bookie LIABILITY (winnings = stake*(odds-1)) at $1000 so a high-odds bet can't stake the full $1000 ('don't put $1000 on a $10 odder'). In _build_afl_tip_from_image's no-units branch, after the stake cap, when odds>1.0: fb_units = min(fb_units, EDDIE_IMAGE_NO_UNITS_MAX_LIABILITY=1000 / (unit_size*(odds-1))). So stake <= 1000/(odds-1): odds=10 -> ~$111 stake ($1000 to-win, NOT $1000 stake); odds=3 -> $500; odds<=2 -> the $1000 stake cap binds first (at the live 1.9, $1000 stake / ~$900 liability). Uses the TIPPED odds; the >1.25x ceiling guard backstops a higher fill. odds=0/missing -> liability cap skipped, stake cap still bounds $1000. The cap only ever REDUCES the stake (min of 2.5u / stake-cap / liability-cap) — never increases or mis-selects. (2) BET-LOG FLAG instead of a maintenance ping (Wilson 'no need for maintenance, just make it clear in betlog'): REMOVED the v5.42 _route_image_afl_tips notify_info; notifier.notify_tip_placed_summary now shows a '⚠️ FALLBACK SIZING — image had NO unit size; assumed Nu (capped $1000 stake / $1000 liability)' tag in the BET PLACED header when tip._units_fallback. The per-image over-parse guard (v5.42) is unchanged. (3) ANYTHING-ELSE FIND — AUDIT.JSONL TEST POLLUTION (same class as the v5.41 bet_ledger X1/X2 bug): the suite was writing Soligo/Cook fixture rows into the production logs/audit.jsonl via _audit_tip; now _audit_tip redirects to a temp file under TIPBOT_TESTING (production audit trail unchanged when the env is unset). 6 existing fixture rows left in place (audit is observational, not the money ledger; the ledger is clean). tests test_eddie_no_units_fallback (extended: odds=3 $500 / odds=10 liability-capped / odds=1.9 stake-cap / no-odds) + test_eddie_fallback_betlog_tag + test_audit_tip_test_isolation. 2-lens adversarial pass. main.py + notifier.py shared -> synced to fork (Eddie inert there). PRIOR v5.42: EDDIE NO-UNITS FALLBACK (Wilson). When an Eddie AFL image tip has NO readable unit sizing (the vision parse returns units null/0 — e.g. Eddie posts the units in a SEPARATE follow-up message we don't merge), the bot now falls back to 2.5u (Eddie's typical play) instead of 1u, CAPPED so the TOTAL stake can't exceed $1000. Trigger: 2026-06-08 Collingwood v Melbourne under 180.5 was a 2.5u play but the vision read no units -> defaulted to 1u -> placed $400 of the intended $1000 (fully filled at the wrong size, so no unfilled alert flagged it). FIX in main._build_afl_tip_from_image (the eddie_afl image builder — eddie_afl is the only 'afl' entry in IMAGE_TIP_PARSERS): new constants EDDIE_IMAGE_NO_UNITS_FALLBACK_UNITS=2.5 + EDDIE_IMAGE_NO_UNITS_MAX_STAKE=1000.0; the no-units branch sets units = min(2.5, 1000/unit_size) so units*unit_size <= $1000 (at the live $400/u that is exactly 2.5u=$1000; a larger unit_size scales the fallback units DOWN to the cap), + a diagnostic log line. ONLY fires when units are unreadable — a correctly-read 1u/3u tip places its REAL size (3u=$1200, uncapped; the cap is fallback-only). Racing build (_build_racing_tip_dict, Zak/Trial) UNCHANGED — its own units=float(default_units) is untouched. HARDENED after a 3-lens adversarial pass (GO with mitigations applied): (a) PER-MESSAGE OVER-PARSE GUARD — the vision can over-read a betslip grid into several tips; with the $1000 fallback that would auto-stake $1000 EACH, so >1 no-units tip from ONE image (the over-parse signature) now routes the guessed-size tips to MANUAL instead (a LONE no-units tip still auto-places 2.5u, Wilson's intended case); (b) HEADS-UP — a no-units fallback placement fires a notify_info so a misread of a genuine 1u tip is VISIBLE (not a silent $1000). _build_afl_tip_from_image marks tip._units_fallback; _route_image_afl_tips counts no-units raw tips upfront (no_units_count). The $1000 per-bet cap + unfilled->manual accounting are unchanged. tests test_eddie_no_units_fallback + test_eddie_overparse_no_units_guard. PRIOR v5.41: TWO fixes (Wilson). (1) RACING TARGET-ODDS FLOOR HARDENED — the per-account placement target_odds (min fill odds sent to the bookie) was current_odds*0.90, measured off the live QUOTE; when the quote sat just above the price-shop floor, a further slip at placement could FILL below the TIPSTER floor (2026-06-08 Valentino re-send: quoted 1.60, target 1.44, filled 1.55 — below the 1.575 tipster floor; it should NOT have placed). New racing_placer._target_odds(current_odds, tipster_odds)=round(max(quote,tipster)*0.90,2) so a fill can NEVER land >10% below the TIPSTER price (a too-low fill is rejected -> manual); a better-than-tipped quote keeps its own 10% slip room; tipster_odds 0/None falls back to quote*0.90 (no regression). Verified: the price-shop already filters quotes to >=tipster*0.9, so the ONLY newly-rejected fills are those between tipster*0.9 and the old quote*0.9 (the intended tightening) — no at-or-above-tipster bet is newly rejected; an odds reject stays CB-neutral + lands in failures->unfilled->manual (adversarial pass SOUND). test_racing_target_odds_floor. Racing path, tipbot-only (the fork has no racing_placer). (2) BET-LEDGER TEST ISOLATION + CSV CLEANUP — the unit suite was POLLUTING the production logs/bets_placed.csv with fixture rows (bet_id X1/X2, $114 Soligo/Cook) on every run (notify_* tests incidentally trigger a ledger write; the path was hard-coded) — 402 phantom rows (~72% of the file), GROWING on every commit. bet_ledger._default_ledger_path() now: BET_LEDGER_PATH env override wins; else TIPBOT_TESTING (set at the top of test_afl_sgm_fixes.py before importing main) forces a temp file; else the production path. So the suite can never touch the real ledger again. Stripped the 402 X1/X2 rows from the live CSV (backup logs/bets_placed.csv.prebak_v5.41; 216 real rows kept). bet_ledger.py is SHARED -> synced to the fork (its suite polluted its ledger too). test_bet_ledger_test_isolation. PRIOR v5.40: TIP TITANS CONTENT-DEDUP — accidental re-send guard (Wilson). A tipster re-sending the same racing card double-placed real money: 2026-06-08 OC's Pinjarra card was re-sent ~6min later and VALENTINO R2 placed $800@1.70 + $800@1.55 ($1600 on an $800 unit) and IDEAL DIAMOND R3 placed $600@2.35 + $600@2.20 ($1200 on $600) — ~$1445 unintended (the rest of the re-send wave was saved only by odds-drift / 'Liability Reached' / 'Insufficient funds'). ROOT CAUSE: tiptitans_processor deduped on the Tip Titans TIP ID, but a re-send is minted a BRAND-NEW id (62109-62114 were re-sends of 62103-62108), so the id-dedup saw them as new and re-placed; the v5.22 _racing_recent_fps content fingerprint only guards the Zak/Trial image+text channels, NOT the Tip Titans path. FIX: a CONTENT fingerprint in tiptitans_processor.process_tip keyed on (titan, track, race#, saddle, market, date) — ANCHORED ON THE SADDLE NUMBER, not the runner name (the re-sent Ideal Diamond was logged 'IDEAL DIAMOMD', a typo a name-keyed fp would miss; saddle #12 was identical), falling back to the runner name only when the saddle is absent (so two no-saddle runners in one race can't collide). A re-send whose fingerprint was placed within TIPTITANS_DEDUP_WINDOW_SEC (default 1800=30min, Wilson's call) is SKIPPED with an INFO 'duplicate re-send SUPPRESSED' maintenance alert (distinguishable from a missed tip); the fingerprint is registered ONLY after a real/maybe-landed placement (placed_stake>0 or ambiguous_outcomes), so a tip that placed $0 (all bookies limited / below floor / no prices) stays re-triable — no false dedup. Serial tip processing -> no lock. New config TIPTITANS_DEDUP_WINDOW_SEC. test_tiptitans_content_dedup (8 assertions: typo case + no-saddle fallback + market/saddle distinctness + window prune); full suite green. Tipbot-only (racing path; the sportsbot fork is sports-only and does not run Tip Titans). NOTE (separate, not in this change): the Valentino re-send filled at 1.55 — below the tipster floor 1.575 — because placement target_odds is quote*0.90 measured off the drifted quote, not the tipster price; a max(quote,tipster)*0.90 floor would close that small gap. PRIOR v5.39: STARTUP LOG BANNER trimmed to the version NUMBER + fingerprint (Wilson) — the file-log startup line was dumping the full multi-KB TIPBOT_VERSION changelog every boot; now it logs just `=== TipBot v5.39 | code fingerprint XXX ===`. The full changelog stays in config.py (this string, the audit/source record) + CHANGELOG.md; the Telegram startup ping was already number-only since v5.19. Log/cosmetic only — no money-path, parser, or placement change. PRIOR v5.38: UNIFIED CONCURRENT SGM FAN-OUT (Saiyan AFL + Shook MLB HRRBI) — Wilson. SGMs now place CONCURRENTLY (ThreadPoolExecutor), EVEN-SPLITTING the unit across the SGM-capable accounts, each capped by its yaml `sgm` LIABILITY ladder (afl.sgm [400,300,200] / mlb.sgm [130,100,87]) sized off the ESTIMATED combined SGM odds = PRODUCT of the per-leg catalog odds (an SGM returns no price pre-place). NEW _place_sgm_fanout + _sgm_fanout_place_account (mirror the AFL singles fan-out) + _sgm_est_combined_odds; _enrich_sgm_legs_with_prop_ids gained an odds_out side-channel (per-leg catalog odds, NEVER on the wire). The even-split's liability lands below the top bracket so the cap normally doesn't bind — it ladders DOWN on a stake-too-high reject or caps a long-odds SGM; the product OVER-states a positively-correlated SGM's true odds so the derived stake UNDER-sizes (the safe side; bookie MBL backstops). DISPATCH: place_tip routes AFL is_sgm -> the fan-out (gated SGM_CONCURRENT_FANOUT, default ON); NBA SGMs stay sequential _place_sgm_v4; _place_mlb_hrrbi runs the fan-out (_orchestrated=True, NO own summary) across the 3 SGM accounts then Alex (65463) as the single-account backstop for the remainder, owning the ONE consolidated summary. RETIRES the v5.15 MLB $100 stake-prepend: _place_sgm_v4's list-cap branch now sizes [400,300,200]/[130,100,87] as LIABILITIES off est_odds too (so the sequential fallback + the SGM_CONCURRENT_FANOUT=false revert stay correct), with the no-est-odds ceiling seeded at the SMALLEST bracket (conservative, not the top). On any non-happy-path (team+total SGM, no priority/sessions, legs-not-carried, no est-odds) the fan-out DELEGATES to the sequential _place_sgm_v4 (the proven manual-routing path). sessions.yaml afl.sgm 600->[400,300,200] (4 SB accts) + mlb.sgm [87,85,80]->[130,100,87] (3 SGM accts) — sessions.yaml is tipbot-only (NOT synced; the sportsbot fork uses its own per-account caps with the same shared code). VERIFIED by a 5-lens money-path adversarial pass (18 raw -> 11 confirmed, 8 money) + a 3-agent fix-verification (all_correct): FIXED (a) MLB orchestrator AMBIGUOUS DOUBLE-PLACE — Alex's remainder now subtracts the maybe-landed SGM at-risk stake + an ambiguous-only outcome SUPPRESSES the full manual re-place (the Erasmus/Dawson guard the rest of the codebase enforces); (b) the no-est-odds over-stake ($130 raw -> $87 conservative ceiling); (c) the stale concurrent_bookies timing flag (now =SGM_CONCURRENT_FANOUT so the concurrent SGM block shows MAX not SUM). ACCEPTED RESIDUALS (documented): the SEQUENTIAL _place_sgm_v4 fallback (off-by-default) retains the pre-existing ambiguous-accounting gap; reconcile.decide_ambiguous is NOT wired into the fan-out worker (consistent with the AFL fan-out — the ladder-stop already prevents the double-place + the ambiguous CRITICAL alert owns reconciliation). Tests test_sgm_fanout (14 assertions: even-split/est-odds=product/ladder-down/no-over-stake/orchestrated-no-summary/delegation) + test_sgm_est_combined_odds + test_sgm_no_odds_ceiling + extended test_mlb_hrrbi_orchestrator (ambiguous-committed + ambiguous-only-suppress); suite green. PRIOR v5.37: PER-STEP TIMING BREAKDOWN + Alex MLB $100 first rung (Wilson). (1) TIMING — every BET PLACED / BET UNFILLED summary now shows a TRUE end-to-end (from the ms the tip ARRIVED at the Telegram handler -> last placement) RECONCILED into named phases that SUM to it: parse / resolve / price-check / bookies(place-wall) / other. New shared notifier._render_timing_block; a tip._timing dict carries t0+parse_sec (stamped at the 3 entry handlers: text _process_tip, image _route_image_afl_tips, racing _route_image_racing_tips), place_tip ADDS resolve_sec (wraps resolve_event on its own timer), the AFL/ETR fan-out ADDS price_check_sec (the resolve-once catalog span). place-wall = MAX(concurrent fan-out) / SUM(sequential) from v5.35. Falls back BYTE-IDENTICAL to the v5.35 bookies/other split when no t0 is present (direct/legacy/test calls). Wired into notify_tip_placed_summary + notify_tip_unfilled_with_placements (new total_elapsed_sec/concurrent_bookies kwargs; the fan-out unfilled callers pass concurrent_bookies=True) + notify_tiptitans_placed (new phase_timing kwarg — the Zak/Trial racing path threads t0+vision/text parse so racing also shows a true end-to-end; the Tip Titans channel passes None -> UNCHANGED). The MLB HRRBI orchestrator now builds session_timing from its placements + passes total_elapsed_sec so its consolidated summary shows a real parse/resolve/bookies/other split (not all-time-in-other). DISPLAY-ONLY — a 5-lens multi-agent adversarial pass confirmed NO money-path impact (4 low findings, all addressed: MLB session_timing wired, yaml comment clarified). test_render_timing_block (reconciliation + legacy-fallback byte-identical). (2) ALEX MLB $100 — sessions.yaml 65463 mlb.player_stats [87,85,80] -> [100,87,85,80] so Alex's 2+ HRRBI SINGLE matches the SGM accounts' v5.15 $100 RUNTIME front-load: the 3 SGMs ($300) + Alex ($100, hard-capped at the ~$100 spillover remainder via _place_mlb_alex_single) now fill the full $400 unit, killing the recurring $13->manual leftover ($387/$400 today). yaml-only money tweak, can never over-stake (capped at remainder). sessions.yaml is tipbot-only (not synced). NEXT (Deploy 2, queued): UNIFIED concurrent SGM fan-out for BOTH Saiyan AFL + Shook MLB HRRBI — even-split the unit across the SGM-capable accounts, each capped by a per-account LIABILITY ladder (afl.sgm [400,300,200] / mlb.sgm [130,100,87]) sized off estimated combined SGM odds = PRODUCT of the per-leg catalog odds; retires the v5.15 $100 stake-prepend; MLB orchestrator calls the fan-out then Alex as the single-account backstop.) PRIOR v5.36: SGM ONE-MESSAGE CONSOLIDATION (Wilson). A saiyan SGM spilling across 2 accounts emitted TWO 'BET PLACED' Telegram messages (per-account notify_bet_placed at the 4 _place_sgm_v4 success sites); now it emits ONE consolidated notify_tip_placed_summary (rolls up every account, stake-weighted avg odds, per-account timing, unfilled tag) — like the AFL fan-out + the MLB orchestrator already do. The summary is emitted by _emit_sgm_aux_alerts (the UNIVERSAL terminal hook), once-guarded by _sgm_summary_sent, so it also covers the EARLY full-fill spillover returns (which return before the function tail) — a tail-only emit would have sent ZERO messages on a fully-filled spillover (caught + fixed pre-ship by a 3-agent adversarial verification). Mechanism: _place_sgm_v4 (non-orchestrated) sets tip._sgm_consolidate=True; notify_bet_placed no-ops on that flag (suppressing the per-account message AND its per-leg ledger write); the tail summary writes the per-leg bets_placed.csv rows (notifier.py:613 hook) so the LEDGER is preserved + sends one message. SEQUENTIAL spillover so concurrent_bookies=False (bookie span = SUM). MLB orchestrated path UNTOUCHED (not flagged; it owns its own summary). Legacy _place_sgm (6062) UNTOUCHED (different path). test_sgm_consolidate_suppresses_per_placement; suite green. NEXT (queued, full build map in memory): full per-step timing breakdown (parse/resolve/place-wall/other, reconciling, all tips) + SGM concurrent placement w/ [400,300,200] per-account liability cap (total ~same; money-path). PRIOR v5.35: concurrent-timing display fix + roster rename (Wilson). (1) CONCURRENT-TIMING DISPLAY — the BET PLACED summary showed '(bookies <SUM>s, other 0.0s)' for the AFL/ETR fan-outs because it SUMMED the per-account elapsed times, but the fan-out places CONCURRENTLY (ThreadPoolExecutor) so the bookie-phase wall-clock is the SLOWEST account (MAX), not the sum — summing parallel work overstated 'bookies' and drove 'other' negative -> clamped to 0.0. notify_tip_placed_summary gained a concurrent_bookies flag: MAX for the fan-out callers (_place_afl_fanout, _place_etr_nba_fanout pass concurrent_bookies=True), SUM unchanged for the sequential paths (singles/SGM/MLB) + racing (notify_tiptitans_placed untouched). So 'other' now reconciles to real parse/resolve/overhead instead of 0.0. (2) ROSTER RENAME — roster_afl.json 'Brad Hill' -> 'Bradley Hill' (Sportsbet's formal name), so the Eddie tip exact-matches the catalog directly (the v5.33 _afl_canonical_catalog_player resolver still covers the general short<->full case, e.g. Matt/Matthew). suite green. NEXT (queued): SGM one-message consolidation + full per-step timing breakdown (parse/resolve/place-wall/other, all tips) + SGM concurrent placement w/ [400,300,200] liability cap. PRIOR v5.34: THREE follow-ups on v5.33 (Wilson). (1) MLB ACCENT-FOLD LOOKUP — exact_match_player now retries accent-insensitively for MLB (roster._fold_accents strips diacritics), so an ASCII Shook tip 'Jose Alvarado'/'Yandy Diaz' with NO team matches the accented MLB Stats API roster key 'José Alvarado'/'Yandy Díaz' and infers the team instead of missing -> manual. Still EXACT on the folded form (no fuzzy/partial -> no wrong-player drift); an accent-only cross-team collision (≈impossible) is guarded by requiring a single team; MLB-only (NBA/AFL exact lookups unchanged). (2) MBL VIOLATION SUPPRESS-WHEN-FILLED — the CRITICAL 'MBL VIOLATION (sports)' alert was firing on a normal code-538 ladder-down that FULLY FILLED (saiyan SGM $600/$600 at 09:03 + 09:05). New _should_alert_mbl_violation gates BOTH the SGM (_emit_sgm_aux_alerts, on remaining_stake + _orchestrated) and singles (_place_singles_v4, on unfilled) alert sites: fire ONLY on a GENUINE shortfall (unfilled > $1 deadband) AND not orchestrated (the MLB per-account model leaves expected leftover); a fully-filled / orchestrated bet logs an INFO 'benign ladder-down' instead of a CRITICAL. Mirrors the racing CB treating 'stake too high' as benign; PRESERVES the $600 cap upside (no cap reduction — Wilson's two options were suppress-when-filled vs lower-cap; chose suppress to keep the bigger stakes the ladder lands where MBL allows). (3) WEEKLY MLB ROSTER REFRESH — new scheduled task TipBot-MLBRosterUpdate (Mon 04:00) runs update_mlb_roster.ps1 (python roster.py --update-mlb + commit-if-changed; post-commit hook auto-pushes) so roster_mlb.json stays current with trades/call-ups; loads on the next TipBot-Main restart, fork syncs on next deploy. register_mlb_roster_task.ps1 (idempotent) registers it. Tests test_mlb_accent_fold_lookup + test_mbl_violation_suppressed_when_filled; suite green. 5-agent adversarial verification. PRIOR v5.33: TWO player-name robustness fixes (Wilson). (1) MLB NO-TEAM ROSTER FALLBACK — when Shook OMITS the team (e.g. 'Matt Olson 2+ HRRBI', 2026-06-07 15:53 routed to manual 'no team'), resolve_event now infers the team from a NEW roster_mlb.json (2028 entries = 1191 players + unambiguous surname aliases, built from the MLB Stats API statsapi.mlb.com; team names are ESPN-compatible e.g. 'Atlanta Braves' so resolve_mlb_event substring-matches them) via the PLAYER name (exact match first since Shook sends full names, else a GUARDED fuzzy requiring a shared >=3-char token so it can never drift to a different player), then resolves the fixture. Fires ONLY when no team is announced — a STATED team is never overridden. roster.py: MLB_ROSTER_FILE + _mlb_roster + _load_rosters + the 4 sport-branch sites + update_mlb_roster_from_api (run: python roster.py --update-mlb). (2) AFL CATALOG NAME-MATCH — _catalog_lookup matched the player by EXACT lowercased string, but the AFL roster canonicalises 'Bradley Hill'->'Brad Hill' (score 0.945) while Sportsbet lists the FULL name 'Bradley Hill', so 'Brad Hill over 19.5' (a line that WAS carried — a live catalog probe confirms a full 0.5-increment over ladder per player, e.g. Merrett 18.5..39.5; game bounced 15:15, tip 15:08 = 7min pre-bounce so props were live) routed to manual (15:08 Brad Hill BET FAILED). New _afl_canonical_catalog_player resolves the tip player to the catalog's EXACT spelling, SURNAME-anchored + unambiguous (a short/full first-name form Brad<->Bradley / Matt<->Matthew is ONLY the collision tiebreak, >=3-char prefix); never guesses a different player. Called in _match_afl_player_prop before the line lookups, so it covers AFL singles AND SGM legs. Tests test_afl_catalog_name_match + test_mlb_no_team_roster_fallback (incl. the exact failure cases below); suite green. The 5-agent adversarial verification FOUND 2 wrong-bet regressions in the first cut — (a) the MLB no-team path used an UNFILTERED fuzzy that drifted a bare/partial name ('Soto') to a same-surname player on the WRONG team; (b) _afl_canonical_catalog_player resolved a lone same-surname player with NO first-name check (Archer Reid->Harley Reid) + a generic >=3-char prefix treated Jack/Jackson, Sam/Samuel as the same. BOTH HARDENED before ship: MLB inference is now EXACT full-name match only (NO fuzzy, >=2-token gate); AFL requires first-name compatibility for EVERY candidate via a curated short<->full nickname allowlist (_AFL_FIRST_NAME_GROUPS — Brad<->Bradley IN, Jack/Jackson OUT, no generic prefix). roster_mlb.json surname aliases are accent-normalised so 'Diaz'/'Díaz' count as one (ambiguous -> no alias). Re-verified 3/3 closed, money_path_ok. PRIOR v5.32: EDDIE 2-HOUR GAME LOOK-AHEAD — the Eddie bare-surname resolver now scans AFL games up to 2 HOURS ahead (EDDIE_GAME_LOOKAHEAD_SEC=7200, was the resolver default 2700=45min), passed explicitly to afl_games_in_play from _resolve_eddie_surname_to_player. Root cause: the 2026-06-07 14:24 Eddie batch (Mills/Wilson/McInerney/Owens) ALL went to manual with 'no AFL game in the start window' because the games bounced >45min after the post. The roster team-scoping + surname-anchor + same-surname odds-tie-break + manual-on-any-ambiguity logic are UNCHANGED — only the look-ahead widened; in-progress games (the 3h behind window) are unchanged. A wider window can surface more same-surname candidates but the existing collision safety (odds tie-break, else manual; never guess a $400 bet) still applies. resolver.py docstring notes the Eddie override. test_eddie_surname_2hr_lookahead (asserts ahead_sec=7200 + a game 90min out now resolves vs the old 45min miss); existing test_eddie_surname_matcher unchanged (its fake_games mock already accepts ahead_sec). 5-agent adversarial verification. PRIOR v5.31: ETR NBA TIPSTER — blind concurrent fan-out. New Telegram group ETR (-4974425383, bot 7738582293) posts leetspeak-OBFUSCATED NBA player props ('Lu2e Ko2net (SAS) ABOVE 1.5 Points' => Luke Kornet over 1.5). New _place_etr_nba_fanout: BLIND (no price-check — leg resolved by the pure _resolve_leg_for_hyperbot transform + POSTed; NBA props place with no proposition_id; target_odds=None => fills at any price, odds IGNORED), FIXED stake ladder [100,90,80,70] ($400 unit /4 = $100 each, then 90/80/70 on a stake reject) across the 4 sportsbet accounts (ETR_NBA_SESSION_IDS 65465/53522/65463/68723), concurrent ThreadPoolExecutor, per-account ladder-down, unfilled->Manual Bets — all inherited from the AFL fan-out. Dispatch: ETR multi/SGM -> manual (SINGLES ONLY); the singles gate (ETR_NBA_CONCURRENT_FANOUT + tipster etr_nba + sport nba) calls the blind fan-out; etr_nba force-bookie sportsbet + ignore-suggested-bookie + TIPSTERS_MAX_ODDS_MULT 0.0 (odds ceiling off; parser also emits odds=0). Groq SYSTEM_PROMPT de-obfuscates leetspeak -> real NBA name (Kev's map + context digit-for-letter), ABOVE/BELOW -> over/under, stat phrase -> stat (incl PRA/PR/PA compounds + 'Reounds'->rebounds), odds=0 forced; Groq-only + roster fuzzy backstop. LIVE-validated: Lu2e Ko2net->Luke Kornet points 1.5 over, Dyl@n H@rp3r->Dylan Harper PRA 21.5 under, both odds=0, both roster-matched to player_points/player_pra. Launched LIVE at $400 (ETR_NBA_TEST_MODE=false, Wilson's call). HARDENING from the 5-agent adversarial pass: (1) single-leg guard (a misparsed multi-leg -> manual, no silent leg-drop); (2) market WHITELIST (an unmapped stat leaves market='player_prop' sentinel -> routes to manual instead of a doomed blind POST). ACCEPTED residual: wrong-player (odds guards off + no price-check, roster team-scope is the only backstop) — Wilson's tradeoff, first placements watched. Re-delivery double-bet covered by the existing text-path dedup. test_etr_nba_fanout; suite green 517/0. 6-agent research + 5-agent adversarial verification. PRIOR v5.30: AFL OVER-LADDERS — direction-aware over/under liability caps (AFL_OVERS_HANDOFF Tasks A-D). A live price_check probe of today's 2 AFL games on Sportsbet confirmed the catalog: the ONLY *_threshold market is goalscorer_threshold_afl; every other stat's OVER lives in its BASE player_* O/U market (dir=over). So over ladders are applied at SIZING (the liability cap), NEVER by routing to a non-existent market (the v5.27 regression this avoids — verified liability_market reaches only resolve_stake_steps, never the bet payload). TASK A: _place_afl_fanout flips liability_market to the *_threshold cap key for an OVER (disposals over -> [300,250,200,150]) while under/ambiguous stays the base O/U cap [124,99,74,50]; OVER detected EXPLICITLY (sel=='over' / endswith ' over' / tip._is_threshold, AND 'under' not in sel) so ambiguous->under (smaller, safer); gated by all() eligible sessions carrying the cap so a missing yaml key never leaves an over UNCAPPED; placement market unchanged. TASK B: 8 more stat over-ladder cap keys on all 4 sportsbet accounts (65465/53522/65463/68723) + KNOWN_AFL_MARKETS — goals goalscorer_threshold_afl [300,250,200] (a DIRECT key; the _threshold_afl normaliser would else strip it to a non-existent sibling -> uncapped), fantasy [249,199,149], marks/tackles/kicks/handballs/clearances/hitouts [125,100]; for 8/9 stats these are INTERNAL sizing keys (the over places on the base player_* market), only goals' key is a real market; text 'fantasy' stat aliased to the fantasy ladder. TASK C: Eddie same-team surname COLLISION (Harley+Archer Reid) now odds-tie-breaks via the live catalog — price each candidate's prop for the tipped stat+line+side, resolve to the one within ~20% of the tip odds, ONLY when EXACTLY ONE is in range AND every candidate was priced (missing-from-catalog -> manual; never guess a $400 bet); legacy 2-arg callers still route collisions to manual. TASK D: verified the AFL fan-out does NOT integer-floor/strand cents (no change). 5-agent design + 5-agent adversarial verification; suite green 502/0. PRIOR v5.29: racing spillover no longer places sub-$1 'finishing' bets + allows DECIMAL stakes (Wilson: 'allow the last bet of the spillover to place with decimals rather than a whole extra bet to finish off the last bit'). New racing_placer._round_spillover_stake: a spillover bet >= $1 rounds to CENTS (was integer-floored), so a SINGLE bet absorbs the fractional remainder (e.g. $373.68) instead of flooring to $373 and stranding the $0.68 onto a separate tiny bet on the next bookie (tonight's MYSTA $0.68 pattern); and a sub-$1 stake that CANNOT reach the $1 min is SKIPPED rather than placed — no more pointless tiny bets that also fail tab's $1 fixed-odds minimum (the JORDANO $0.73 / Felix $0.05 pattern); a sub-$1 TEST stake with room still rounds UP to $1. The <$1 remainder is left unfilled (negligible). test_round_spillover_stake. (The AFL fan-out decimal-remainder is a separate path — floors at AFL_FANOUT_MIN_STAKE so it doesn't go sub-$1; tracked in AFL_OVERS_HANDOFF.md task D.) PRIOR v5.28: REVERT v5.27's disposals-overs-threshold-only routing — it was a live regression. A catalog probe (sportsbet 53522, 2 upcoming games) confirmed Sportsbet carries NO separate disposals/marks/etc threshold market (ONLY goalscorer_threshold_afl for goals): a disposals OVER ('23+'/over) lives in the BASE player_disposals market (selection = bare player NAME, direction=over; an UNDER is 'Player Under' in the SAME market). v5.27 had routed disposals overs to player_disposals_threshold ONLY, and since that market does not exist, every disposals over was going to MANUAL (live 21:16; no AFL games in the window so it never actually bit). _match_afl_player_prop now restores the base-O/U-over fallback. NET: disposals overs place again on player_disposals (over side) at the player_disposals cap [124,99,74,50] (shared with unders for now). The desired OVER-specific ladder [300,250,200,150] + the other-stat over ladders + collision odds-check + decimal-remainder fix are a HANDOFF to the next session — see AFL_OVERS_HANDOFF.md (the over/under cap SPLIT needs a direction-aware liability_market in _place_afl_fanout because over+under share the placement market). KEPT from v5.27 (pre-staged for the handoff): the player_disposals_threshold yaml cap key, the _threshold_afl cap-lookup normalisation, the KNOWN_AFL_MARKETS entry. PRIOR v5.27: AFL disposals UNDER vs OVER now use SEPARATE liability ladders + overs are THRESHOLD-ONLY (Wilson). sessions.yaml (4 sportsbet accts 65465/53522/65463/68723): player_disposals = [124,99,74,50] (UNDER / base O/U market; 125->124 + 100->99 from v5.26's rung) and a NEW explicit player_disposals_threshold = [300,250,200,150] (OVER / 'X+' market — Sportsbet allows far higher liability on the threshold market than the O/U line). (1) The explicit _threshold key OVERRIDES the old sibling-fallback (which made threshold inherit player_disposals). (2) session_priority.lookup_liability_cap normalises a '_threshold_afl' suffix -> '_threshold' so BOTH the HyperBot market name (player_disposals_threshold_afl) AND the internal one (player_disposals_threshold) resolve to the explicit cap — previously the _afl form fell through to None (uncapped), a real risk now thresholds ladder to $300. (3) _match_afl_player_prop: DISPOSALS overs bet the THRESHOLD market ONLY — the base player_disposals over-line fallback is REMOVED; a missing threshold prop snaps to the nearest threshold +/-1.0, else None -> manual (other AFL stats unchanged). KNOWN_AFL_MARKETS gained player_disposals_threshold. test_afl_disposals_split_caps (caps for all 3 market forms + over/under routing + no-base-over-fallback). PRIOR v5.26: AFL player_disposals fan-out ladder gained a $125-LIABILITY TOP rung across all 4 sportsbet accounts (65465/53522/65463/68723) in sessions.yaml: [100,74,50] -> [125,100,74,50] (Wilson). Each account now tries $125 liability FIRST (~$147 stake @1.85, ~$184 @1.68), then ladders down to the existing 100/74/50 ($117/$87/$58) on a stake-too-high reject. $125 liability sits in the historically-rejected $125-185 stake band, so on most accounts/games it'll stake-too-high and ladder down to $117 (= prior behaviour) — it only lands the bigger stake where the live MBL allows. Pure upside, no downside (rejects ladder down; benign fast 538 rejects, don't trip anything). Covers player_disposals O/U + threshold (X+) via the sibling-fallback key. yaml-only money-path change; version bumped for the audit trail + a verifiable restart. PRIOR v5.25: Eddie AFL bare-surname player props now resolve to the AFL game ABOUT TO START. Eddie posts last-name-only props at game time ('Daniel 25+ disposals'); a bare surname previously mis-resolved (get_player_team fuzzy-matched 'Daniel'->Daniel Turner, a FIRST-name hit) or went to manual. NEW _resolve_eddie_surname_to_player (main.py): finds the AFL game(s) about-to-start/in-progress near the POST time via resolver.afl_games_in_play (Squiggle q=games `unixtime` window — no timezone reasoning), then surname-anchors the token to a UNIQUE player on those teams via roster.afl_surname_candidates scoped by resolver.team_key (bridges Squiggle short names e.g. 'Adelaide' -> roster 'Adelaide Crows'). Daniel->Caleb Daniel (NM playing); Bailey->Zac Bailey (Brisbane playing). Game-scoping collapses league-wide ambiguity (Daniel/Bailey are common FIRST names) to ~unique within the two teams; FIRST-name matches are NEVER used; a collision / two in-play teams sharing the surname / no game in the window / not-in-roster all -> MANUAL (never guesses a $400 bet; no fuzzy fall-back for bare tokens). The eddie_afl vision prompt now EMITS a surname-only player (was told to never output just 'Daniel') so the matcher can resolve it. NO Claude API / web search needed (Squiggle + roster_afl.json cover it). test_eddie_surname_matcher (11 assertions). ALSO LIVE since the 19:01 .env restart: IMAGE_TIPS_TEST_MODE=false + EDDIE_UNIT_SIZE=400 — Eddie AFL flipped from the $1/u test stake to FULL $400/u (Wilson); only affects Eddie (the sole afl image parser); Zak/Trial stay on the separate IMAGE_RACING_TEST_MODE. PRIOR v5.24: racing circuit-breaker now cools a flaky bookie SESSION after ONE session-level failure (RACING_CB_FAIL_STREAK 3->1, in both the default and .env), and on a PLACEMENT failure as well as a price-check (previously only price-checks fed the breaker). A single tab/bet365 timeout or 'betslip enquiry failed'/'Failed to get odds' now cools that session for 300s (auto-retries + resets on the first good price-check) instead of dragging ~17-30s off every subsequent tip while Soup fixes the daemon. Placement failures now call _cb_record in BOTH the PLACE FAIL and PLACE EXCEPTION branches; _CB_FAILURE_MARKERS broadened (betslip / enquiry failed / failed to get odds / bet placement failed) while FAST stake/odds rejects (code 538 'stake too high') stay BENIGN so a normal MBL ladder-down never trips the breaker. Wilson's call after a controlled test proved tab/bet365's placement path is intermittently down server-side (not our payload — same runner_match + target_odds as the bot's successful 16:26 placements). test_racing_circuit_breaker pins N=3 for the consecutive-streak/reset/stale-window cases + asserts the v5.24 single-fail trip and the new placement markers. PRIOR v5.23: forensic-batch fixes from a 7-agent placement audit. (1) Saiyan HANDICAP bets (SGM OR single) -> MANUAL (Wilson) via _tip_has_handicap_leg + a place_tip guard. The Fremantle +0.5 SGM leg had broken with 'over not found' because the SGM same-player carry-over leaked a player into the stat-less handicap leg, so it masqueraded as a player prop and evaded the handicap->manual guard; the carry-over now ONLY fires for a leg that has a STAT. (2) saiyan_afl added to TIPSTERS_IGNORE_SUGGESTED_BOOKIE so a tip Saiyan quoted at a non-Sportsbet bookie (O'Driscoll @ Bet365) still places on Sportsbet via the fan-out instead of going to manual. (3) _promote_misparsed_sgms now requires 2+ DISTINCT STATS to merge singles into an SGM — a same-player SAME-stat ladder (AusBets 'Fox over 14.5 P / over 15.5 P') stays as independent singles (was wrongly merged into a 2-leg SGM that placed nothing). (4) IMAGE_PROMPT_AFL tightened to require the player's FULL name + team (Eddie vision dropped 'Caleb' from 'Caleb Daniel' -> bare surname mis-resolved -> manual). ALSO LIVE since the 14:25 .env restart: ZAK_UNIT_SIZE/TRIAL_SNIPER_UNIT_SIZE 10->400 (Zak/Trial were under-staking 40x). PRIOR v5.22: HARDENS the v5.21 Zak/Trial text path after a 5-agent adversarial review (no money emergency found; these close the real gaps). (1) RACING-PATH DEDUP — the racing path (image AND text) had NO dedup, so a reposted/re-delivered tip DOUBLE-PLACED real money; added a fingerprint skip (tipster,track,race#,runner,saddle,market,date) within DUPE_WINDOW_SECS in _route_image_racing_tips, registered BEFORE placing (covers image + text). (2) parse_racing_text now RAISES on a HARD failure (Groq down / bad JSON) so _process_text_racing_tip routes the tip to MANUAL instead of silently DROPPING it (was a lost-tip regression — the 'never lose a tip' fallback was dead code); a valid-but-empty parse still returns [] -> chatter -> drop. (3) RESULTS GUARD — _text_looks_like_result routes a results/recap post ('R7 Lingani WON at 4.50') to MANUAL before parsing, so a settled race cannot auto-place. Tests: test_racing_dedup + results/parse-fail assertions. PRIOR v5.21: (1) Zak/Trial TEXT-TIP PLACEMENT — actionable TEXT posts on the racing image channels (e.g. 'Adding Lingani for tomorrow') are now PARSED (new groq_parser.parse_racing_text, same racing schema the vision path emits) and routed through the SAME racing pipeline as images, so a genuine text tip PLACES at the live unit size. The relative-date helper (_img_parse_racing_date: tomorrow/6-6/weekday), runner-match, price floor/ceiling, 3u image cap, and runner-only/no-race# -> manual (Guard 2) ALL apply unchanged. Chatter (parser returns no runner) is DROPPED with no manual ping; a parse ERROR falls back to a manual alert so a real tip is never lost. AFL image channel (Eddie) keeps the old manual-alert behaviour. (2) bets_placed.csv LEDGER — new bet_ledger.py writes ONE row per LANDED bet (19 cols, CSV for Excel Power Query) at every placement-success across ALL paths (sports via notify_bet_placed + notify_tip_placed_summary, racing via notify_tiptitans_placed); bet_id dedup, fully guarded so a logging failure can NEVER break a placement; closes the audit coverage gap (racing + MLB previously wrote no machine-readable placement record). PRIOR v5.20: AFL fan-out UNFILLED now = the FULL gap vs the intended UNIT (intended - placed - maybe-landed), routed to Manual Bets. PRIOR (v5.13) a ladder-DOWN was 'expected, not unfilled' so the remainder was silently dropped (Tim English placed $340 of a $600 unit -> shown as 'placed $340 of $340', $260 never flagged). Wilson wants the WHOLE remainder placed by hand: BOTH the ladder-down shortfall AND the part the 4 SB accounts' liability brackets can't hold. Ambiguous (maybe-landed) stake stays COMMITTED (excluded from unfilled, not re-prompted). Applies to ALL AFL fan-out (Saiyan + Eddie). $1 deadband. PRIOR v5.19: Telegram STARTUP PING shows the version NUMBER only (TIPBOT_VERSION.split(' (')[0]), not the full changelog — the growing TIPBOT_VERSION string overflowed Telegram's 4096-char limit (HTTP 400 'message is too long' on the v5.18 startup notify). The file-log banner still records the full TIPBOT_VERSION. Cosmetic; no money-path change. PRIOR v5.18: MLB HRRBI leftover->manual GUARD against stale/replayed tips. A sportsbot fork instance (running the v5.16 code synced today) emitted FOUR identical Freddie Freeman 'BET UNFILLED' alerts — Event UNRESOLVED, empty Raw, '@ 0' odds, 'no error captured', $348/$400 — for an overnight Shook tip re-delivered on a flaky telethon reconnect (OUTSIDE the 10-min dedup window). This tipbot (v5.15) did NOT send them. FIX: _place_mlb_hrrbi now only fires the leftover->manual alert (the v5.16 Happ fix) when the tip is GENUINELY live — require a resolved tip.event OR a non-empty tip.raw_message as proof-of-liveness; a degraded replay (both empty) logs a warning and is suppressed instead of spamming Manual Bets. Root cause (re-delivery dedup window) noted as a follow-up. PRIOR v5.17: Eddie AFL HALF/QUARTER period guard — Eddie tips a 'Hawthorn -5.5 2nd Half Line' but the vision dropped '2nd Half' and it was treated as a FULL-GAME -5.5 line (caught only by the odds floor -> manual today, but would mis-place if the price matched). FIX: the eddie_afl vision prompt now captures a `period` field (1st/2nd half, quarters), and _build_afl_tip_from_image routes ANY non-full-game period to manual (the bot's AFL catalog is full-game only) — applies to lines, totals AND player props. PRIOR v5.16: forensic fixes — (a) MLB player-match Tucker bug: _resolve_mlb_player returned None silently (sub-0.82 fuzzy or ambiguity-tie) so 'Kyle Tucker' routed to manual though carried; added a SURNAME ANCHOR (unique catalog surname + compatible first name -> match the variant, never drift to a different player) + logged the previously-silent return-None branches. (b) MLB HRRBI leftover now ALSO fires a Manual Bets alert (notify_tip_unfilled_with_placements), not just the inline 'Unfilled $X' tag (Happ's $52 went unsent). (c) AFL_SGM_SESSION_PRIORITY=65465,53522,68723 — added Wilson 53522 + Daniel 68723 as SGM spillover partners (both afl.sgm:600, SGM-capable) so AFL SGM overflow rolls over instead of all-to-manual. PRIOR v5.15: (1) MLB HRRBI SGM tries a $100 STAKE rung FIRST, then the existing [87,85,80] ladder (MLB-scoped prepend in _place_sgm_v4; no pre-place SGM odds so 100 is a stake step, not a converted liability). (2) Zak/Trial racing-image plays flipped to FULL unit sizing ($400/u): fixed a bug where the production stake used the channel's default_units (1.0) instead of unit_size (so it never actually staked at the configured size); added a SEPARATE IMAGE_RACING_TEST_MODE gate (default false=live) so Zak/Trial go full-size WITHOUT un-testing Eddie AFL (still $1/u via IMAGE_TIPS_TEST_MODE). The 10% odds floor, runner-match, 1.5x wrong-horse ceiling, route-remainder/whole-to-manual, and 3u per-runner cap already existed (shared with Tip Titans). Builds on v5.14.) === ORIGINAL v5.14: AFL fan-out re-enables the WRONG-SELECTION ceiling. After the v5.13 scrutiny flagged that the odds ceiling was also a wrong-selection guard (a catalog-valid-but-wrong pick — same-surname / ±1.0 line / wrong-O/U snap — was placing across ALL accounts), Wilson chose 'ceiling only'. _resolve_single_for_placement's apply_odds_guards flag is split into apply_ceiling + apply_floor; the fan-out now resolves with apply_ceiling=True, apply_floor=False — so a live price > 1.25x tipped routes the whole tip to manual (off the resolve-time catalog odds, no extra call), while a shorter-than-tipped live price still places. All other paths (NBA/MLB/handicap/total/SGM/racing via presolved=None) keep BOTH guards (defaults). Residual (accepted): longer-fill liability overshoot + h2h-no-odds uncapped sizing still rely on bookie MBL. Builds on v5.13.)"
