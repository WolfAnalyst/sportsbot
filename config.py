...
TIPBOT_VERSION = "v5.56 (2026-06-13: ALL 5 AUDIT REVIEW-ITEMS (Wilson: all 5). 1) notifier._send ONE retry on transient failure (exception/5xx/429) + NOTIFY LOST (final) grep needle on definitive loss (one failed POST used to silently drop CRITICALs); parse-entities plain fallback kept + exception-guarded. 2) reconcile pending_bet_matches selection-token HARD gate: a pending bet naming a DIFFERENT selection no longer matches (v5.54 concurrent twins on one race/account could confirm each other); token-level so name-format drift stays safe; soft only on empty bet text. 3) reconcile DEFAULT_MAX_WAIT_SEC 20->30s (trader-review landings past the window edge). 4) xwatcher_watchdog.ps1: ensure log dir + %TEMP% fallback (was totally silent on unwritable log path). 5) deploy.ps1 prints staged files, flags UNTRACKED in yellow before add -A.)"
