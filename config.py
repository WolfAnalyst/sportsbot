...
# top-up. When the fan-out leaves stake unfilled (an account failed e.g. on low
# balance) AND other accounts placed, re-split the unfilled remainder EVENLY over
# the accounts that worked and top each up (100/90/80/70% ladder, stop on reject)
# — time-sensitive AFL overs fill on the bookies that worked instead of -> manual.
# Disposals-OVER singles ONLY (unders/SGMs excluded — harder liability). Kill-switch.
EDDIE_OVERS_REDISTRIBUTE = _env_bool("EDDIE_OVERS_REDISTRIBUTE", True)
