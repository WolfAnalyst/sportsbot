...
TIPBOT_VERSION = "v5.55 (2026-06-13: 12-AGENT AUDIT FIXES (Wilson; report AUDIT_2026-06-12.md; 51 raised -> 16 verified real -> 7 fixed). 1) racing PRE_PLACEMENT_REJECT_PATTERNS drift: added line-moved/did-not-match (in main.py since v4.6) so slow racing slip-validation rejects spill cleanly instead of blind-debiting as AMBIGUOUS; parity test pins both modules. 2) TT content-dedup race under v5.54 concurrency: atomic _claim_bet/_release_bet claims the fingerprint immediately before placement (two identical re-sends in one batch can no longer both place); $0 tips release (v5.40 semantics); image racing tips get the same claim (they had NO content dedup). 3) reconcile-CONFIRMED-placed now converts to a full SUCCESS at the actual stake (fan-outs + alex single) -> placed accounting/summary/ledger instead of the ambiguous at-risk bucket; alex not_placed -> no debit, leftover->manual. 4) tiptitans_state.json TIPBOT_TESTING temp isolation. 5) racing ambiguous records carry correlation_id for /v3/transactions audit trail.)"
