...
def _fix_sgm_threshold_leg(leg: ParsedLeg, is_pyo_sgm: bool, sport: str = "nba") -> ParsedLeg:
    ...
    # v5.69 (m16) + r2 (#8): NEVER let an 'under' leg be a threshold market.
    # Threshold markets are inherently N+ (over) and downstream routing discards
    # the direction, so "Player under 30 pts" placed as a threshold = "30+", the
    # WRONG SIDE. The guard must CLEAR a stale _is_threshold (both build loops set
    # it from the model's raw is_threshold BEFORE calling this, so a
    # contradictory {selection:'under', is_threshold:true} leg would otherwise
    # reach placement still flagged) — not merely skip a new promotion. Leave it
    # as an O/U under leg at the integer line.
    if (leg.selection or "").strip().lower() in ("under", "u"):
        leg._is_threshold = False
        return leg
...
IMAGE_PROMPT_AFL = (
    ...
    "the first and do NOT collapse the list. CRITICAL: the SAME player can appear "
    "on SEVERAL lines with DIFFERENT lines / thresholds (e.g. 'Will Ashcroft over "
    "26.5 disposals' AND 'Will Ashcroft 30+ disposals'; or 'Greene 2+ goals' AND "
    "'Greene 3+ goals' AND 'Greene 4+ goals') — output a SEPARATE tip for EACH "
    "such line, and read the units/stake AND odds from THAT line ONLY. NEVER "
    "merge two lines into one tip, and NEVER take the line/threshold from one row "
    "but the units or odds from a different row. NEVER output both the "
    ...
    "competing team in `team`. WINNING MARGIN: a TEAM name (NOT a player) "
    ...
    "handicap line. EITHER-TEAM / TRIBET BAND: some margin bands are NOT tied to "
    "one team — 'TriBet', 'Either Team 1-24', 'Match Margin 1-24', '1-24 Any Team' "
    "mean the MATCH is decided by a margin INSIDE the range regardless of which side "
    "wins. Set market_type=\"margin\", `team`=null, `side`=\"over\", `line`=null, and "
    "`description`=the EXACT text as printed (e.g. \"Either Team 1-24\" / \"TriBet "
    "1-24\"). Do NOT force a team onto an either-team band, and NEVER emit "
    "market_type=\"other\" for a margin band. Anything else \"other\". "
